...
long Triangles::run(
    ...
    JasmineGraphHashMapCentralStore centralStore,
    JasmineGraphHashMapDuplicateCentralStore duplicateCentralStore,
    std::string graphId, std::string partitionId, int threadPriority) {
  ...
  map<long, unordered_set<long>> duplicateCentralDBSubGraphMap =
      duplicateCentralStore.getUnderlyingHashMap();
  ...
  map<long, long> centralDBDegreeDistribution =
      centralStore.getOutDegreeDistributionHashMap();
  map<long, long> centralDuplicateDBDegreeDistribution =
      duplicateCentralStore.getOutDegreeDistributionHashMap();
  ...
  for (centralDuplicateDBDegreeDistributionIterator =
    ...
    long centralDuplicateDBStartVid =
        centralDuplicateDBDegreeDistributionIterator->first;

    unordered_set<long> centralDBSecondVertexSet =
        centralDBSubGraphMap[centralDuplicateDBStartVid];
    unordered_set<long> duplicateSecondVertexSet =
        duplicateCentralDBSubGraphMap[centralDuplicateDBStartVid];
    std::set<long> result;

    std::set_difference(
        duplicateSecondVertexSet.begin(), duplicateSecondVertexSet.end(),
    ...
    if (result.size() > 0) {
      ...
      centralDBSubGraphMap[centralDuplicateDBStartVid].insert(result.begin(),
                                                              result.end());
    }
  }

  // Merging Local Store and Workers central stores before starting triangle
  // count
  for (centralDBDegreeDistributionIterator =
           centralDBDegreeDistribution.begin();
    ...
    localSubGraphMap[centralDBStartVid].insert(
        centralDBSubGraphMap[centralDBStartVid].begin(),
        centralDBSubGraphMap[centralDBStartVid].end());
  }

  auto mergeEnd = std::chrono::high_resolution_clock::now();
  auto mergeDur = mergeEnd - mergeBbegin;
  auto mergeMsDuration =
      std::chrono::duration_cast<std::chrono::milliseconds>(mergeDur).count();

  triangle_logger.log(
      " Merge time Taken: " + std::to_string(mergeMsDuration) + " milliseconds",
      "info");
  ...
  for (iterator = degreeMap.begin(); iterator != degreeMap.end(); ++iterator) {
    ...
    for (verticesIterator = vertices.begin();
         verticesIterator != vertices.end(); ++verticesIterator) {
      long temp = *verticesIterator;
      std::set<long> orderedUList(localSubGraphMap[temp].begin(),
                                  localSubGraphMap[temp].end());
      std::set<long>::iterator uListIterator;
      for (uListIterator = orderedUList.begin();
           uListIterator != orderedUList.end(); ++uListIterator) {
        long u = *uListIterator;
        std::set<long> orderedNuList(localSubGraphMap[u].begin(),
                                     localSubGraphMap[u].end());
        std::set<long>::iterator nuListIterator;
        for (nuListIterator = orderedNuList.begin();
             nuListIterator != orderedNuList.end(); ++nuListIterator) {
          long nu = *nuListIterator;
          if ((localSubGraphMap[temp].find(nu) !=
               localSubGraphMap[temp].end()) ||
  ...
  triangle_logger.log("###TRIANGLE### Triangle Counting: Completed: Triangles" +
                          std::to_string(triangleCount),
                      "info");
  return triangleCount;
}

string Triangles::countCentralStoreTriangles(
    map<long, unordered_set<long>> centralStore,
  ...
  for (iterator = degreeMap.begin(); iterator != degreeMap.end(); ++iterator) {
    ...
    for (verticesIterator = vertices.begin();
      ...
      for (uListIterator = uList.begin(); uListIterator != uList.end();
           ++uListIterator) {
        long u = *uListIterator;
        std::unordered_set<long> nuList = centralStore[u];
        std::unordered_set<long>::iterator nuListIterator;
        for (nuListIterator = nuList.begin(); nuListIterator != nuList.end();
             ++nuListIterator) {
          long nu = *nuListIterator;
          if ((centralStore[temp].find(nu) != centralStore[temp].end()) ||
            ...
            if (itemResIterator != itemRes.end()) {
              ...
              if (std::find(list.begin(), list.end(), varThree) == list.end()) {
                ...
                triangleCount++;
                triangle = triangle + std::to_string(varOne) + "," +
                           std::to_string(varTwo) + "," +
