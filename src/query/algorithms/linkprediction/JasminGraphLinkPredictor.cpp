...
void JasminGraphLinkPredictor::initiateLinkPrediction(std::string graphID,
                                                      std::string path,
                                                      std::string masterIP) {
  JasmineGraphServer *jasmineServer = new JasmineGraphServer();
  std::map<std::string, JasmineGraphServer::workerPartitions>
      graphPartitionedHosts = jasmineServer->getGraphPartitionedHosts(graphID);
  ...
  for (std::map<std::string, JasmineGraphServer::workerPartitions>::iterator
           it = (graphPartitionedHosts.begin());
    ...
    if (count == 0) {
      ...
      remainHostMap.insert(
          std::pair<std::string, JasmineGraphServer::workerPartitions>(
              it->first, it->second));
    }
    count++;
  }
  ...
  for (std::map<std::string, JasmineGraphServer::workerPartitions>::iterator
    ...
    for (std::vector<std::string>::iterator j =
             ((remainHostMap[it->first].partitionID).begin());
  ...
  this->sendQueryToWorker(selectedHostName, selectedHostPort,
                          selectedHostDataPort, selectedHostPartitionsNo,
                          graphID, vertexCount, path, hostsList, masterIP);
}

int JasminGraphLinkPredictor::sendQueryToWorker(
    std::string host, int port, int dataPort, int selectedHostPartitionsNo,
  ...
  predictor_logger.log("Sent : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                       "info");
  bzero(data, 301);
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    predictor_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::INITIATE_PREDICT, "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      predictor_logger.log("Received : " + JasmineGraphInstanceProtocol::OK,
                           "info");
      ...
      write(sockfd, to_string(selectedHostPartitionsNo).c_str(),
            to_string(selectedHostPartitionsNo).size());
      predictor_logger.log("Sent : No of partition in selected host " +
                               to_string(selectedHostPartitionsNo),
                           "info");
      ...
      if (response.compare(JasmineGraphInstanceProtocol::SEND_HOSTS) == 0) {
        predictor_logger.log(
            "Received : " + JasmineGraphInstanceProtocol::SEND_HOSTS, "info");
        ...
        if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_NAME) ==
            0) {
          predictor_logger.log(
          ...
          if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_LEN) ==
            ...
            predictor_logger.log("Sent : File length in bytes " + fileLength,
                                 "info");
            bzero(data, 301);
            read(sockfd, data, 300);
            response = (data);
            ...
            if (response.compare(
              ...
              predictor_logger.log(
                  ...
                  "info");
              predictor_logger.log("Going to send file through service",
                                   "info");
              JasmineGraphServer::sendFileThroughService(
                  host, dataPort, fileName, filePath, masterIP);
            }
          }
        }
        int count = 0;
        while (true) {
          ...
          if (response.compare(JasmineGraphInstanceProtocol::FILE_RECV_WAIT) ==
            ...
            predictor_logger.log(
                ...
                "info");
            predictor_logger.log("Checking file status : " + to_string(count),
                                 "info");
            count++;
            sleep(1);
            continue;
