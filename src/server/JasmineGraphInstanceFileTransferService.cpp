...
void *filetransferservicesession(void *dummyPt) {
  filetransferservicesessionargs *sessionargs =
      (filetransferservicesessionargs *)dummyPt;
  ...
  string filePathWithName = utils.getJasmineGraphProperty(
                                "org.jasminegraph.server.instance.datafolder") +
                            "/" + fileName;

  write(connFd, JasmineGraphInstanceProtocol::SEND_FILE.c_str(),
        JasmineGraphInstanceProtocol::SEND_FILE.size());
  int bytesReceived = 0;
  char buffer[1024];
...
void JasmineGraphInstanceFileTransferService::run(int dataPort) {
  ...
  // TODO :: What is the maximum number of connections allowed?? Considered as
  // 100 for now
  while (connectionCounter < 100) {
    file_service_logger.log(
        "Worker FileTransfer Service listening on port " + to_string(dataPort),
        "info");
    connFd = accept(listenFd, (struct sockaddr *)&clntAdd, &len);

    if (connFd < 0) {
      file_service_logger.log(
          "Cannot accept connection to port " + to_string(dataPort), "error");
    } else {
      file_service_logger.log(
          "Connection successful to port " + to_string(dataPort), "info");
      struct filetransferservicesessionargs filetransferservicesessionargs1;
      filetransferservicesessionargs1.connFd = connFd;

      pthread_create(&threadA[connectionCounter], NULL,
                     filetransferservicesession,
