...
class JasmineGraphInstanceProtocol {
 public:
  // Handshaking is the first task that JasmineGraph's main server does with an
  // JasmineGraph Instance once it gets connected. During the phase of
  // Handshaking, JasmineGraph server informs its host name to the instance so
  // that it can connect with the server later time.
  ...
  static const string
      BATCH_UPLOAD_CENTRAL;  // This is to upload centralstore file as a batch
  static const string
      BATCH_UPLOAD_COMPOSITE_CENTRAL;  // This is to upload composite
                                       // centralstore file as a batch
  static const string
      UPLOAD_RDF_ATTRIBUTES;  // This is to upload attribute list of partitions
                              // file as a batch
  static const string
      UPLOAD_RDF_ATTRIBUTES_CENTRAL;     // This is to upload attribute list of
                                         // centralstore file as a batch
  static const string BATCH_UPLOAD_CHK;  // This is to check whether the upload
  ...
  static const string SEND_FILE_LEN;  // This is to indicate server to send the
                                      // size of the file.
  static const string
      SEND_FILE_CONT;  // This is to indicate server to send the file contents.
  static const string SEND_FILE_COMPLETE;
  static const string SEND_FILE_NAME;
  static const string
      SEND_PARTITION_ID;  // This command is used by the Instance service
                          // session to ask for partition id.
  static const string
      SEND_PARTITION_ITERATION;  // This command is used by the Instance service
                                 // session to ask the training iteration in
                                 // which the partition should train.
  static const string
      SEND_PARTITION_COUNT;  // This command is used by the Instance service
                             // session to ask the total partitions expected to
                             // be trained inside a host.
  ...
  static const string
      STATUS;  // This is sent to the client to check its status.
  static const string DELETE_GRAPH;  // This message deletes a particular graph
                                     // from JasmineGraph
