...
void *runInstanceService(void *dummyPt) {
  ...
  refToInstance->instanceService->run(
      refToInstance->profile, refToInstance->masterHostName,
      refToInstance->hostName, refToInstance->serverPort,
...
int JasmineGraphInstance::start_running(string profile, string hostName,
                                        string masterHost, int serverPort,
  ...
  pthread_create(&instanceFileTransferThread, NULL, runFileTransferService,
                 this);
...
bool JasmineGraphInstance::acknowledgeMaster(string masterHost, string workerIP,
                                             string workerPort) {
  int sockfd;
  char data[301];
  bool loop = false;
  ...
  graphInstance_logger.log("###INSTANCE### Get Host By Name : " + masterHost,
                           "info");
  ...
  graphInstance_logger.log("Sent : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                           "info");
  bzero(data, 301);
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
      ...
      result_wr = write(
          sockfd, JasmineGraphInstanceProtocol::ACKNOWLEDGE_MASTER.c_str(),
      ...
      graphInstance_logger.log(
          "Sent : " + JasmineGraphInstanceProtocol::ACKNOWLEDGE_MASTER, "info");
      bzero(data, 301);
      read(sockfd, data, 300);
      response = (data);
      response = utils.trim_copy(response, " \f\n\r\t\v");

      if (response.compare(JasmineGraphInstanceProtocol::WORKER_INFO_SEND) ==
          0) {
...
void JasmineGraphInstance::startNmonAnalyzer(string enableNmon,
                                             int serverPort) {
  Utils utils;
  if (enableNmon == "true") {
    std::string nmonFileLocation = utils.getJasmineGraphProperty(
        "org.jasminegraph.server.nmon.file.location");
    std::string numberOfSnapshots =
        utils.getJasmineGraphProperty("org.jasminegraph.server.nmon.snapshots");
    std::string snapshotGap = utils.getJasmineGraphProperty(
        "org.jasminegraph.server.nmon.snapshot.gap");
    std::string nmonFileName =
        nmonFileLocation + "nmon.log." + std::to_string(serverPort);
    std::string nmonStartupCommand = "nmon_x86_64_ubuntu18 -c " +
                                     numberOfSnapshots + " -s " + snapshotGap +
                                     " -T -F " + nmonFileName;
    ...
    if (input) {
      ...
      if (!result.empty()) {
        graphInstance_logger.log("Error in performance database backup process",
                                 "error");
      }
...
bool JasmineGraphInstance::sendFileThroughService(std::string host,
                                                  int dataPort,
                                                  std::string fileName,
                                                  std::string filePath) {
  Utils utils;
