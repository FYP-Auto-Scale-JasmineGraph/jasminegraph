...
void *runfrontend(void *dummyPt) {
  ...
  refToServer->frontend = new JasmineGraphFrontEnd(
      refToServer->sqlite, refToServer->performanceSqlite,
...
int JasmineGraphServer::run(std::string profile, std::string masterIp,
                            int numberofWorkers, std::string workerIps,
  ...
  if (masterIp.empty()) {
    this->masterHost =
        utils.getJasmineGraphProperty("org.jasminegraph.server.host");
  } else {
  ...
  myThreads[0] =
      std::thread(StatisticCollector::logLoadAverage, "Load Average");
  sleep(2);
...
void JasmineGraphServer::start_workers() {
  ...
  if (profile == "native") {
    ...
    if ((this->numberOfWorkers) == -1) {
      nWorkers =
          utils.getJasmineGraphProperty("org.jasminegraph.server.nworkers");
    }
    enableNmon =
        utils.getJasmineGraphProperty("org.jasminegraph.server.enable.nmon");
  ...
  for (it = hostsList.begin(); it < hostsList.end(); it++) {
    ...
    hostString += "(" + std::to_string(counter) + ", '" + ip_address + "', '" +
                  ip_address + "', 'false'),";
    counter++;
  }
  ...
  for (it = hostsList.begin(); it < hostsList.end(); it++) {
    string sqlStatement =
        "INSERT INTO worker "
        "(idworker,host_idhost,name,ip,user,is_public,server_port,server_data_"
        "port) VALUES ";
    string valuesString;
    ...
    while (portCount < numberOfWorkersPerHost) {
      ...
      hostPortMap.insert((pair<string, pair<int, int>>(
          *it, make_pair(workerPort, workerDataPort))));
      portCount++;
      // ToDO: Here for the moment we use host name as the IP address as the
      // third parameter. ToDO: We also keep user as empty string
      string is_public = "false";
      valuesString += "(" + std::to_string(workerIDCounter) + ", " + hostID +
                      ", \"" + hostName + "\", \"" + ip + "\",\"" + user +
                      "\", '" + is_public + "',\"" +
  ...
  for (hostListIterator = hostsList.begin(); hostListIterator < hostsList.end();
       hostListIterator++) {
    ...
    myThreads[count] = std::thread(startRemoteWorkers, workerPortsMap[host],
                                   workerDataPortsMap[host], host, profile,
...
void JasmineGraphServer::waitForAcknowledgement(int numberOfWorkers) {
  ...
  while (timeDifference < Conts::JASMINEGRAPH_WORKER_ACKNOWLEDGEMENT_TIMEOUT) {
    ...
    std::string selectQuery =
        "select idworker from worker where status='started'";
    std::vector<vector<pair<string, string>>> output =
        this->sqlite.runSelect(selectQuery);
...
void JasmineGraphServer::startRemoteWorkers(
    std::vector<int> workerPortsVector, std::vector<int> workerDataPortsVector,
    string host, string profile, string masterHost, string enableNmon) {
  Utils utils;
  std::string executableFile;
  std::string workerPath =
      utils.getJasmineGraphProperty("org.jasminegraph.worker.path");
  std::string artifactPath =
      utils.getJasmineGraphProperty("org.jasminegraph.artifact.path");
  std::string instanceDataFolder = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.instance.datafolder");
  std::string aggregateDataFolder = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.instance.aggregatefolder");
  std::string nmonFileLocation = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.nmon.file.location");
  std::string jasmineGraphExecutableName = Conts::JASMINEGRAPH_EXECUTABLE;
  server_logger.log(
      "###MASTER#### Starting remote workers for profile " + profile, "info");
  ...
  if (artifactPath.empty() ||
      artifactPath.find_first_not_of(' ') == artifactPath.npos) {
  ...
  if (profile == "native") {
    ...
    for (int i = 0; i < workerPortsVector.size(); i++) {
      if (host.find("localhost") != std::string::npos) {
        serverStartScript =
            executableFile + " native 2 " + host + " " + masterHost + " " +
            std::to_string(workerPortsVector.at(i)) + " " +
            std::to_string(workerDataPortsVector.at(i)) + " " + enableNmon;
      } else {
        serverStartScript = "ssh -p 22 " + host + " " + executableFile +
                            " native 2 " + host + " " + masterHost + " " +
                            std::to_string(workerPortsVector.at(i)) + " " +
    ...
    for (int i = 0; i < workerPortsVector.size(); i++) {
      if (masterHost == host || host == "localhost") {
        serverStartScript =
            "docker run -v " + instanceDataFolder + ":" + instanceDataFolder +
            " -v " + aggregateDataFolder + ":" + aggregateDataFolder + " -v " +
            nmonFileLocation + ":" + nmonFileLocation + " -v " +
            ...
            std::to_string(workerDataPortsVector.at(i)) + ":" +
            std::to_string(workerDataPortsVector.at(i)) +
            " jasminegraph:" + image_tag + " --MODE 2 --HOST_NAME " + host +
            " --MASTERIP " + masterHost + " --SERVER_PORT " +
            std::to_string(workerPortsVector.at(i)) + " --SERVER_DATA_PORT " +
            std::to_string(workerDataPortsVector.at(i)) + " --ENABLE_NMON " +
            enableNmon;
      } else {
        serverStartScript =
            "docker -H ssh://" + host + " run -v " + instanceDataFolder + ":" +
            instanceDataFolder + " -v " + aggregateDataFolder + ":" +
            aggregateDataFolder + " -v " + nmonFileLocation + ":" +
            nmonFileLocation + " -v " + instanceDataFolder + "/" +
            to_string(i) + "/logs" + ":" +
            "/home/ubuntu/software/jasminegraph/logs" + " -p " +
            std::to_string(workerPortsVector.at(i)) + ":" +
            std::to_string(workerPortsVector.at(i)) + " -p " +
...
bool JasmineGraphServer::spawnNewWorker(string host, string port,
                                        string dataPort, string profile,
                                        string masterHost, string enableNmon) {
  SQLiteDBInterface refToSqlite = *new SQLiteDBInterface();
  refToSqlite.init();
  string selectHostSQL = "SELECT idhost from host where name='" + host + "'";
  string selectWorkerSQL = "SELECT idworker from worker where server_port = '" +
                           port + "' or server_data_port = '" + dataPort + "'";
  std::vector<vector<pair<string, string>>> checkWorkerOutput =
      refToSqlite.runSelect(selectWorkerSQL);

  if (checkWorkerOutput.size() > 0) {
    return false;
  }

  std::vector<vector<pair<string, string>>> selectHostOutput =
      refToSqlite.runSelect(selectHostSQL);
  string idHost = "";
  ...
  if (selectHostOutput.size() > 0) {
    ...
    std::vector<vector<pair<string, string>>> selectMaxHostOutput =
        refToSqlite.runSelect(maxHostIDSQL);
    idHost = selectMaxHostOutput[0][0].second;

    int hostId = atoi(idHost.c_str());
    hostId++;
    std::string hostInsertString =
        "INSERT INTO host (idhost,name,ip,is_public) VALUES ('" +
  ...
  std::vector<vector<pair<string, string>>> selectMaxWorkerIdOutput =
      refToSqlite.runSelect(maxWorkerIDSQL);
  ...
  string workerInsertSqlStatement =
      ...
      "(idworker,host_idhost,name,ip,user,is_public,server_port,server_data_"
      "port) VALUES ('" +
      to_string(maxWorkerId) + "','" + idHost + "','" + host + "','" + host +
      "','','false','" + port + "','" + dataPort + "')";
  ...
  startRemoteWorkers(workerPortsVector, workerDataPortsVector, host, profile,
                     masterHost, enableNmon);
...
void JasmineGraphServer::resolveOperationalGraphs() {
  string sqlStatement =
      "SELECT partition_graph_idgraph,partition_idpartition,worker_idworker "
      "FROM worker_has_partition ORDER BY worker_idworker";
  std::vector<vector<pair<string, string>>> output =
      sqlite.runSelect(sqlStatement);
  std::map<int, vector<string>> partitionMap;

  for (std::vector<vector<pair<string, string>>>::iterator i = output.begin();
       i != output.end(); ++i) {
    int workerID = -1;
    string graphID;
    string partitionID;
  ...
  for (map<int, vector<string>>::iterator it = partitionMap.begin();
       it != partitionMap.end(); ++it) {
    int len = (it->second).size();
    int workerID = it->first;

    for (std::vector<string>::iterator x = (it->second).begin();
         x != (it->second).end(); ++x) {
  ...
  for (std::vector<vector<pair<string, string>>>::iterator i = output.begin();
    ...
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                      "info");
    bzero(data, 301);
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
      ...
      result_wr = write(
          sockfd,
          JasmineGraphInstanceProtocol::INITIATE_FRAGMENT_RESOLUTION.c_str(),
      ...
      response = (data);
      response = utils.trim_copy(response, " \f\n\r\t\v");
      server_logger.log("Received : " + JasmineGraphInstanceProtocol::OK,
                        "info");

      if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
        std::vector<string> partitionList = partitionAggregatedMap[workerID];

        for (std::vector<string>::iterator x = partitionList.begin();
             x != partitionList.end(); ++x) {
          ...
          if (response.compare(
                  JasmineGraphInstanceProtocol::FRAGMENT_RESOLUTION_CHK) == 0) {
            continue;
          } else {
            server_logger.log(
                "Error in fragment resolution process. Received : " + response,
                "error");
          }
        }
        ...
        if (response.compare(
          ...
          result_wr = write(
              sockfd,
              JasmineGraphInstanceProtocol::FRAGMENT_RESOLUTION_DONE.c_str(),
          ...
          server_logger.log(
              "Sent : " +
                  JasmineGraphInstanceProtocol::FRAGMENT_RESOLUTION_DONE,
              "info");
          ...
          if (response.compare("") != 0) {
            ...
            for (std::vector<string>::iterator it = listOfOitems.begin();
                 it != listOfOitems.end(); it++) {
  ...
  std::vector<vector<pair<string, string>>> output2 =
      sqlite.runSelect(sqlStatement);
  std::set<int> graphIDsFromMetDBSet;
  for (std::vector<vector<pair<string, string>>>::iterator i = output2.begin();
       i != output2.end(); ++i) {
    std::vector<pair<string, string>>::iterator j = (i->begin());
    graphIDsFromMetDBSet.insert(atoi(j->second.c_str()));
  }

  for (std::set<int>::iterator itr = graphIDsFromWorkersSet.begin();
       itr != graphIDsFromWorkersSet.end(); itr++) {
...
void JasmineGraphServer::deleteNonOperationalGraphFragment(int graphID) {
  ...
  for (std::map<string, pair<int, int>>::iterator it = hostPortMap.begin();
       it != hostPortMap.end(); it++) {
    // Fetch hostname and port
    string hostname = it->first;
    int port = (it->second).first;
    // Initialize threads for host
    // Each thread runs the service to remove the given graph ID fragments in
    // their datafolders
    deleteThreads[count++] =
        std::thread(removeFragmentThroughService, hostname, port,
                    to_string(graphID), this->masterHost);
    sleep(1);
    server_logger.log(
        "Deleted graph fragments of graph ID " + to_string(graphID), "info");
  }
...
void JasmineGraphServer::shutdown_workers() {
  ...
  for (mapIterator = hostWorkerMap.begin(); mapIterator < hostWorkerMap.end();
       mapIterator++) {
    workers worker = *mapIterator;
    Utils utils;
    bool result = true;
    std::cout << pthread_self() << " host : " << worker.hostname
              << " port : " << worker.port << " DPort : " << worker.dataPort
              << std::endl;
    int sockfd;
    char data[301];
    bool loop = false;
    ...
    bzero(data, 301);
    write(sockfd, JasmineGraphInstanceProtocol::SHUTDOWN.c_str(),
          JasmineGraphInstanceProtocol::SHUTDOWN.size());
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::SHUTDOWN,
                      "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    string response = (data);
...
void JasmineGraphServer::uploadGraphLocally(
    int graphID, const string graphType,
  ...
  while (count < total_threads) {
    ...
    for (mapIterator = hostWorkerMap.begin(); mapIterator < hostWorkerMap.end();
      ...
      workerThreads[count] =
          std::thread(batchUploadFile, worker.hostname, worker.port,
                      worker.dataPort, graphID, partitionFileName, masterHost);
      count++;
      sleep(1);
      copyCentralStoreToAggregateLocation(centralStoreFileList[file_count]);
      workerThreads[count] =
          std::thread(batchUploadCentralStore, worker.hostname, worker.port,
                      worker.dataPort, graphID,
                      centralStoreFileList[file_count], masterHost);
      count++;
      sleep(1);

      if (compositeCentralStoreFileList.find(file_count) !=
          compositeCentralStoreFileList.end()) {
        copyCentralStoreToAggregateLocation(
            compositeCentralStoreFileList[file_count]);
        workerThreads[count] =
            std::thread(batchUploadCompositeCentralstoreFile, worker.hostname,
                        worker.port, worker.dataPort, graphID,
                        compositeCentralStoreFileList[file_count], masterHost);
        count++;
        sleep(1);
      }
      ...
      workerThreads[count] =
          std::thread(batchUploadCentralStore, worker.hostname, worker.port,
                      worker.dataPort, graphID,
                      centralStoreDuplFileList[file_count], masterHost);
      count++;
      sleep(1);
      if (graphType == Conts::GRAPH_WITH_ATTRIBUTES) {
        workerThreads[count] =
            std::thread(batchUploadAttributeFile, worker.hostname, worker.port,
                        worker.dataPort, graphID, attributeFileList[file_count],
                        masterHost);
        count++;
        sleep(1);
        workerThreads[count] =
            std::thread(batchUploadCentralAttributeFile, worker.hostname,
                        worker.port, worker.dataPort, graphID,
                        centralStoreAttributeFileList[file_count], masterHost);
        count++;
        sleep(1);
      }
      assignPartitionToWorker(partitionFileName, graphID, worker.hostname,
                              worker.port, worker.dataPort);
      file_count++;
    }
  }
  ...
  // The following function updates the 'worker_has_partition' table and 'graph'
  // table only
  updateMetaDB(hostWorkerMap, partitionFileList, graphID, uploadEndTime);
}

void JasmineGraphServer::assignPartitionToWorker(std::string fileName,
                                                 int graphId,
  ...
  std::string workerSearchQuery =
      ...
      "' and server_port='" + std::to_string(workerPort) +
      "' and server_data_port='" + std::to_string(workerDataPort) + "'";

  std::vector<vector<pair<string, string>>> results =
      refToSqlite.runSelect(workerSearchQuery);

  std::string workerID = results[0][0].second;

  std::string partitionToWorkerQuery =
      "insert into worker_has_partition (partition_idpartition, "
      "partition_graph_idgraph, worker_idworker) values "
      "('" +
...
bool JasmineGraphServer::batchUploadFile(std::string host, int port,
                                         int dataPort, int graphID,
                                         std::string filePath,
                                         std::string masterIP) {
  Utils utils;
  bool result = true;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
      server_logger.log("Received : " + JasmineGraphInstanceProtocol::HOST_OK,
                        "info");
    } else {
    ...
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::BATCH_UPLOAD,
                      "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_NAME) == 0) {
        ...
        if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_LEN) ==
            0) {
          server_logger.log(
          ...
          server_logger.log("Sent : File length in bytes " + fileLength,
                            "info");
          bzero(data, 301);
          read(sockfd, data, 300);
          response = (data);
          ...
          if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_CONT) ==
              0) {
            server_logger.log(
      ...
      while (true) {
        ...
        read(sockfd, data, 300);
        response = (data);
        if (response.compare(JasmineGraphInstanceProtocol::FILE_RECV_WAIT) ==
            0) {
          server_logger.log(
              "Received : " + JasmineGraphInstanceProtocol::FILE_RECV_WAIT,
              "info");
          server_logger.log("Checking file status : " + to_string(count),
                            "info");
          count++;
          sleep(1);
          continue;
        } else if (response.compare(JasmineGraphInstanceProtocol::FILE_ACK) ==
                   0) {
          server_logger.log(
              "Received : " + JasmineGraphInstanceProtocol::FILE_ACK, "info");
          server_logger.log("File transfer completed for file : " + filePath,
                            "info");
          break;
        }
      };
      ...
      while (true) {
        result_wr = write(
            sockfd, JasmineGraphInstanceProtocol::BATCH_UPLOAD_CHK.c_str(),
        ...
        if (response.compare(JasmineGraphInstanceProtocol::BATCH_UPLOAD_WAIT) ==
            0) {
          server_logger.log(
...
bool JasmineGraphServer::batchUploadCentralStore(std::string host, int port,
                                                 int dataPort, int graphID,
                                                 std::string filePath,
                                                 std::string masterIP) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd, JasmineGraphInstanceProtocol::BATCH_UPLOAD_CENTRAL.c_str(),
    ...
    server_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::BATCH_UPLOAD_CENTRAL, "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
      ...
      while (true) {
        ...
        if (response.compare(JasmineGraphInstanceProtocol::BATCH_UPLOAD_WAIT) ==
          ...
          server_logger.log(
              "Received : " + JasmineGraphInstanceProtocol::BATCH_UPLOAD_ACK,
              "info");
          server_logger.log("CentralStore partition file upload completed",
                            "info");
          break;
        }
      }
...
void JasmineGraphServer::copyCentralStoreToAggregateLocation(
    std::string filePath) {
  Utils utils;
  char buffer[128];
...
bool JasmineGraphServer::batchUploadAttributeFile(std::string host, int port,
                                                  int dataPort, int graphID,
                                                  std::string filePath,
                                                  std::string masterIP) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd, JasmineGraphInstanceProtocol::UPLOAD_RDF_ATTRIBUTES.c_str(),
    ...
    server_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::UPLOAD_RDF_ATTRIBUTES,
        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
...
bool JasmineGraphServer::batchUploadCentralAttributeFile(std::string host,
                                                         int port, int dataPort,
                                                         int graphID,
                                                         std::string filePath,
                                                         std::string masterIP) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::UPLOAD_RDF_ATTRIBUTES_CENTRAL.c_str(),
    ...
    server_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::UPLOAD_RDF_ATTRIBUTES_CENTRAL,
        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
...
bool JasmineGraphServer::batchUploadCompositeCentralstoreFile(
    std::string host, int port, int dataPort, int graphID, std::string filePath,
    std::string masterIP) {
  Utils utils;
  int sockfd;
  char data[301];
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::BATCH_UPLOAD_COMPOSITE_CENTRAL.c_str(),
      ...
      while (true) {
        ...
        if (response.compare(JasmineGraphInstanceProtocol::BATCH_UPLOAD_WAIT) ==
          ...
          server_logger.log(
              ...
              "info");
          server_logger.log(
              "Composite CentralStore partition file upload completed", "info");
          break;
        }
      }
    }
...
bool JasmineGraphServer::sendFileThroughService(std::string host, int dataPort,
                                                std::string fileName,
                                                std::string filePath,
                                                std::string masterIP) {
  Utils utils;
...
void JasmineGraphServer::copyArtifactsToWorkers(std::string workerPath,
                                                std::string artifactLocation,
                                                std::string remoteWorker) {
  if (artifactLocation.empty() ||
      artifactLocation.find_first_not_of(' ') == artifactLocation.npos) {
    server_logger.log(
        "Received `" + artifactLocation + "` for `artifactLocation` value!",
        "error");
    throw std::invalid_argument(
        "Received empty string for `artifactLocation` value!");
  }
  std::string pathCheckCommand =
      "test -e " + workerPath + "&& echo file exists || echo file not found";
  std::string artifactCopyCommand;
  const int ARTIFACTS_COUNT = 4;
  std::string artifactsArray[ARTIFACTS_COUNT] = {"JasmineGraph", "run.sh",
                                                 "conf", "GraphSAGE"};
  ...
  for (int i = 0; i < ARTIFACTS_COUNT; i++) {
    localWorkerArtifactCopyCommandArray[i] = "cp -r " + artifactLocation + "/" +
                                             artifactsArray[i] + " " +
                                             workerPath;
    remoteWorkerArtifactCopyCommandArray[i] = "scp -r " + artifactLocation +
                                              "/" + artifactsArray[i] + " " +
  ...
  if (remoteWorker.find("localhost") == std::string::npos) {
    std::string remotePathCheckCommand =
        "ssh -p 22 " + remoteWorker + " " + pathCheckCommand;
  ...
  for (int i = 0; i < ARTIFACTS_COUNT; i++) {
    ...
    if (copyInput) {
      ...
      if (!result.empty()) {
        server_logger.log(
            "Error executing command for copying worker artifacts : " + result,
            "error");
      }
      pclose(copyInput);
    }
...
void JasmineGraphServer::deleteWorkerPath(std::string workerHost,
                                          std::string workerPath) {
  ...
  if (input) {
    ...
    if (!result.empty()) {
      server_logger.log(
          "Error executing command for deleting worker path : " + result,
          "error");
    }
    pclose(input);
  }
...
void JasmineGraphServer::createWorkerPath(std::string workerHost,
                                          std::string workerPath) {
  ...
  if (input) {
    ...
    if (!result.empty()) {
      server_logger.log(
          "Error executing command for creating worker path : " + result,
          "error");
    }
    pclose(input);
  }
...
void JasmineGraphServer::createLogFilePath(std::string workerHost,
                                           std::string workerPath) {
  ...
  if (input) {
    ...
    if (!result.empty()) {
      server_logger.log(
          "Error executing command for creating log file path : " + result,
          "error");
    }
    pclose(input);
  }
...
void JasmineGraphServer::addHostsToMetaDB(std::string host,
  ...
  for (int i = 0; i < portVector.size(); i++) {
    ...
    if (!utils.hostExists(name, ip_address, std::to_string(workerPort),
                          this->sqlite)) {
      string hostID = Utils::getHostID(name, this->sqlite);
      sqlStatement =
          ("INSERT INTO worker "
           "(host_idhost,name,ip,user,is_public,server_port,server_data_port) "
           "VALUES (\"" +
...
void JasmineGraphServer::updateMetaDB(
    vector<JasmineGraphServer::workers> hostWorkerMap,
    std::map<int, string> partitionFileList, int graphID,
    string uploadEndTime) {
  SQLiteDBInterface refToSqlite = *new SQLiteDBInterface();
  refToSqlite.init();
  string sqlStatement2 = "UPDATE graph SET upload_end_time = '" +
                         uploadEndTime + "' ,graph_status_idgraph_status = '" +
                         to_string(Conts::GRAPH_STATUS::OPERATIONAL) +
                         "' WHERE idgraph = '" + to_string(graphID) + "'";
  refToSqlite.runUpdate(sqlStatement2);
}

void JasmineGraphServer::removeGraph(
    vector<pair<string, string>> hostHasPartition, string graphID,
    std::string masterIP) {
  std::cout << "Deleting the graph partitions.." << std::endl;
  int count = 0;
  std::thread *deleteThreads = new std::thread[hostHasPartition.size()];
  for (std::vector<pair<string, string>>::iterator j =
           (hostHasPartition.begin());
       j != hostHasPartition.end(); ++j) {
    deleteThreads[count] =
        std::thread(removePartitionThroughService, j->first,
                    hostPortMap[j->first].first, graphID, j->second, masterIP);
    count++;
    sleep(1);
  }
...
/** Used to delete graph fragments of a given graph ID for a particular host
 * running at a particular port
 *
 ...
 */
int JasmineGraphServer::removeFragmentThroughService(string host, int port,
                                                     string graphID,
                                                     string masterIP) {
  Utils utils;
  std::cout << pthread_self() << " host : " << host << " port : " << port
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd, JasmineGraphInstanceProtocol::DELETE_GRAPH_FRAGMENT.c_str(),
    ...
    server_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::DELETE_GRAPH_FRAGMENT,
        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
...
int JasmineGraphServer::removePartitionThroughService(string host, int port,
                                                      string graphID,
                                                      string partitionID,
                                                      string masterIP) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::DELETE_GRAPH,
                      "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      if (response.compare(JasmineGraphInstanceProtocol::SEND_PARTITION_ID) ==
          0) {
        server_logger.log(
            "Received : " + JasmineGraphInstanceProtocol::SEND_PARTITION_ID,
            "info");
...
std::vector<JasmineGraphServer::workers>
JasmineGraphServer::getHostWorkerMap() {
...
void JasmineGraphServer::updateOperationalGraphList() {
  ...
  string sqlStatement =
      ("SELECT b.partition_graph_idgraph FROM worker_has_partition AS b "
       "JOIN worker WHERE worker.idworker = b.worker_idworker AND worker.name "
       "IN "
       "(" +
       hosts +
       ") GROUP BY b.partition_graph_idgraph HAVING "
       "COUNT(b.partition_idpartition)= "
  ...
  std::vector<vector<pair<string, string>>> v =
      this->sqlite.runSelect(sqlStatement);
  for (std::vector<vector<pair<string, string>>>::iterator i = v.begin();
       i != v.end(); ++i) {
    for (std::vector<pair<string, string>>::iterator j = (i->begin());
         j != i->end(); ++j) {
      graphIDs += (j->second + ", ");
    }
  }
...
std::map<std::string, JasmineGraphServer::workerPartition>
JasmineGraphServer::getWorkerPartitions(string graphID) {
  vector<pair<string, string>> hostHasPartition;
  SQLiteDBInterface refToSqlite = *new SQLiteDBInterface();
  refToSqlite.init();
  map<std::string, JasmineGraphServer::workerPartition> graphPartitionedHosts;
  vector<vector<pair<string, string>>> hostPartitionResults =
      refToSqlite.runSelect(
          "SELECT name, worker_idworker, server_port, server_data_port, "
          "partition_idpartition FROM worker_has_partition INNER JOIN worker "
          "ON worker_"
          "idworker = idworker WHERE partition_graph_idgraph = '" +
          graphID + "'");

  for (std::vector<vector<pair<string, string>>>::iterator i =
           hostPartitionResults.begin();
    ...
    cout << "name : " << name << " workerID : " << workerID
         << " sport : " << serverPort << " sdport : " << serverDataPort
         << " partitionId : " << partitionId << endl;
    graphPartitionedHosts.insert(
        (pair<string, JasmineGraphServer::workerPartition>(
...
std::map<string, JasmineGraphServer::workerPartitions>
JasmineGraphServer::getGraphPartitionedHosts(string graphID) {
  ...
  vector<vector<pair<string, string>>> hostPartitionResults =
      refToSqlite.runSelect(
          "SELECT name, partition_idpartition FROM worker_has_partition INNER "
          "JOIN worker ON worker_"
          "idworker = idworker WHERE partition_graph_idgraph = '" +
          graphID + "'");
  for (vector<vector<pair<string, string>>>::iterator i =
           hostPartitionResults.begin();
       i != hostPartitionResults.end(); ++i) {
    int count = 0;
    string hostname;
  ...
  for (map<string, vector<string>>::iterator it = (hostPartitions.begin());
       it != hostPartitions.end(); ++it) {
    graphPartitionedHosts.insert(
        (pair<string, JasmineGraphServer::workerPartitions>(
            it->first,
            {hostPortMap[it->first].first, hostPortMap[it->first].second,
...
bool JasmineGraphServer::hasEnding(std::string const &fullString,
...
std::vector<std::string> JasmineGraphServer::getWorkerVector(
...
void JasmineGraphServer::backupPerformanceDB() {
  Utils utils;
  std::string performanceDBPath =
      utils.getJasmineGraphProperty("org.jasminegraph.performance.db.location");
  ...
  std::string backupScript = "cp " + performanceDBPath + " " +
                             performanceDBPath + "-" +
                             to_string(currentTimestamp);
  ...
  if (input) {
    ...
    if (!result.empty()) {
      server_logger.log("Error in performance database backup process",
                        "error");
    }
...
void JasmineGraphServer::addInstanceDetailsToPerformanceDB(
    std::string host, std::vector<int> portVector, std::string isMaster) {
  ...
  std::string searchHost =
      "select idhost,ip from host where ip='" + ipAddress + "'";
  vector<vector<pair<string, string>>> selectResult =
      this->performanceSqlite.runSelect(searchHost);
  ...
  std::string insertPlaceQuery =
      "insert into place "
      "(host_idhost,server_port,ip,user,is_master,is_host_reporter) values ";

  for (it = portVector.begin(); it < portVector.end(); it++) {
    std::string isHostReporter = "false";
    int port = (*it);
    std::string searchPlaceQuery = "select idplace from place where ip='" +
                                   ipAddress + "' and host_idhost='" + hostId +
                                   "' and server_port='" + to_string(port) +
                                   "';";
    vector<vector<pair<string, string>>> placeSearchResult =
        this->performanceSqlite.runSelect(searchPlaceQuery);
    ...
    if (count == 0) {
      std::string searchReporterQuery = "select idplace from place where ip='" +
                                        ipAddress +
                                        "' and is_host_reporter='true'";
      vector<vector<pair<string, string>>> searchResult =
          this->performanceSqlite.runSelect(searchReporterQuery);
      if (searchResult.size() == 0) {
        isHostReporter = "true";
      }
    }
    hostString += "('" + hostId + "','" + to_string(port) + "','" + ipAddress +
                  "','" + user + "','" + isMaster + "','" + isHostReporter +
                  "'),";
    count++;
  }
...
void JasmineGraphServer::inDegreeDistribution(std::string graphID) {
  std::map<std::string, JasmineGraphServer::workerPartition>
      graphPartitionedHosts = JasmineGraphServer::getWorkerPartitions(graphID);
  ...
  for (workerit = graphPartitionedHosts.begin();
       workerit != graphPartitionedHosts.end(); workerit++) {
  ...
  bzero(data, 301);
  int result_wr = write(
      sockfd, JasmineGraphInstanceProtocol::IN_DEGREE_DISTRIBUTION.c_str(),
  ...
  server_logger.log(
      "Sent : " + JasmineGraphInstanceProtocol::IN_DEGREE_DISTRIBUTION, "info");
  bzero(data, 301);
  read(sockfd, data, 300);
  string response = (data);
  response = utils.trim_copy(response, " \f\n\r\t\v");
  ...
  if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      server_logger.log("Sent : Partition ID " + std::to_string(partitionID),
                        "info");
...
void JasmineGraphServer::outDegreeDistribution(std::string graphID) {
  ...
  bzero(data, 301);
  int result_wr = write(
      sockfd, JasmineGraphInstanceProtocol::OUT_DEGREE_DISTRIBUTION.c_str(),
  ...
  server_logger.log(
      "Sent : " + JasmineGraphInstanceProtocol::OUT_DEGREE_DISTRIBUTION,
      "info");
  bzero(data, 301);
  read(sockfd, data, 300);
  string response = (data);
...
void JasmineGraphServer::duplicateCentralStore(std::string graphID) {
  ...
  for (workerit = graphPartitionedHosts.begin();
    ...
    bzero(data, 301);
    int result_wr =
        write(sockfd, JasmineGraphInstanceProtocol::DP_CENTRALSTORE.c_str(),
              JasmineGraphInstanceProtocol::DP_CENTRALSTORE.size());
    if (result_wr < 0) {
      server_logger.log("Error writing to socket", "error");
    }

    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::DP_CENTRALSTORE,
                      "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    string response = (data);
...
void JasmineGraphServer::initiateFiles(std::string graphID,
                                       std::string trainingArgs) {
  int count = 0;
  JasmineGraphTrainingSchedular *schedular =
      new JasmineGraphTrainingSchedular();
  map<string, map<int, int>> scheduleForAllHosts =
      schedular->schedulePartitionTraining(graphID);
  std::map<std::string, JasmineGraphServer::workerPartitions>
      graphPartitionedHosts = this->getGraphPartitionedHosts(graphID);
  int partition_count = 0;
  std::map<std::string, JasmineGraphServer::workerPartitions>::iterator
      mapIterator;
  ...
  Utils utils;
  string prefix = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.instance.trainedmodelfolder");
  string attr_prefix = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.instance.datafolder");
  trainingArgs = trainingArgs;
  std::map<std::string, JasmineGraphServer::workerPartitions>::iterator j;
  for (j = graphPartitionedHosts.begin(); j != graphPartitionedHosts.end();
       j++) {
    ...
    for (k = partitions.begin(); k != partitions.end(); k++) {
      ...
      workerThreads[count] =
          std::thread(initiateTrain, j->first, workerPartition.port,
                      workerPartition.dataPort, trainingArgs + " " + *k,
                      iterationOfPart, partitionCount);
      count++;
      sleep(3);
    }
  }
...
void JasmineGraphServer::initiateCommunication(std::string graphID,
                                               std::string trainingArgs,
                                               SQLiteDBInterface sqlite) {
  Utils utils;
  int fl_clients =
      stoi(utils.getJasmineGraphProperty("org.jasminegraph.fl_clients"));
  ...
  for (int i = 0; i < workerVector.size(); i++) {
    ...
    if (i == 0) {
      workerThreads[threadID] =
          std::thread(initiateServer, "localhost", serverPort, serverDataPort,
                      trainingArgs, fl_clients, to_string(i));
      threadID++;
    }

    workerThreads[threadID] = std::thread(
        initiateClient, "localhost", serverPort, serverDataPort,
...
void JasmineGraphServer::initiateOrgCommunication(std::string graphID,
                                                  std::string trainingArgs,
                                                  SQLiteDBInterface sqlite) {
  Utils utils;
  int fl_clients =
      stoi(utils.getJasmineGraphProperty("org.jasminegraph.fl_clients"));
  int orgs_count =
      stoi(utils.getJasmineGraphProperty("org.jasminegraph.fl.num.orgs"));
  std::string flagPath =
      utils.getJasmineGraphProperty("org.jasminegraph.fl.flag.file");
  int threadLimit = fl_clients + 1;
  ...
  if (utils.getJasmineGraphProperty("org.jasminegraph.fl.aggregator") ==
      "true") {
    std::thread *orgAggThread = new std::thread[1];
    orgAggThread[0] = std::thread(
        initiateAggregator, "localhost", stoi(workerVector[0].port),
        stoi(workerVector[0].dataPort), trainingArgs, fl_clients, "1");
    orgAggThread[0].join();
  }

  std::ifstream file(
      utils.getJasmineGraphProperty("org.jasminegraph.fl.organization.file"));
    ...
    if (file.is_open()) {
      ...
      while (std::getline(file, line)) {
        ...
        if (strArr.size() == 3 && org_thread_count <= orgs_count) {
          trainThreads[org_thread_count] =
              std::thread(sendTrainCommand, host, port, trainingArgs);
          org_thread_count++;
        }
      }
      file.close();
  ...
  communicationThread[0] =
      std::thread(receiveGlobalWeights, "localhost", 5000, "0", 1, "1");
  ...
  for (int i = 0; i < workerVector.size(); i++) {
    ...
    if (i == 0) {
      workerThreads[threadID] =
          std::thread(initiateOrgServer, "localhost", serverPort,
...
void JasmineGraphServer::initiateMerge(std::string graphID,
                                       std::string trainingArgs,
                                       SQLiteDBInterface sqlite) {
  int count = 0;
  JasmineGraphTrainingSchedular *schedular =
  ...
  for (j = graphPartitionedHosts.begin(); j != graphPartitionedHosts.end();
    ...
    for (k = partitions.begin(); k != partitions.end(); k++) {
      ...
      workerThreads[count] = std::thread(
          mergeFiles, "localhost", workerPartition.port,
...
bool JasmineGraphServer::initiateTrain(std::string host, int port, int dataPort,
                                       std::string trainingArgs, int iteration,
                                       string partCount) {
  Utils utils;
  bool result = true;
  int sockfd;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::INITIATE_FILES,
                      "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
    ...
    server_logger.log(
        "There was an error in the invoking training process and the response "
        "is :: " +
            response,
        "error");
  }
...
bool JasmineGraphServer::initiateServer(std::string host, int port,
                                        int dataPort, std::string trainingArgs,
  ...
  write(sockfd, JasmineGraphInstanceProtocol::HANDSHAKE.c_str(),
        JasmineGraphInstanceProtocol::HANDSHAKE.size());
  server_logger.log("Sent fed : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                    "info");
  bzero(data, FED_DATA_LENGTH);
  read(sockfd, data, FED_DATA_LENGTH);
  string response = (data);
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    server_logger.log(
        "Received fed : " + JasmineGraphInstanceProtocol::HANDSHAKE_OK, "info");
    string server_host =
        utils.getJasmineGraphProperty("org.jasminegraph.server.host");
    write(sockfd, server_host.c_str(), server_host.size());
    ...
    server_logger.log(
        "Sent fed : " + JasmineGraphInstanceProtocol::INITIATE_SERVER, "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      server_logger.log("Received fed : " + JasmineGraphInstanceProtocol::OK,
                        "info");
      write(sockfd, (trainingArgs).c_str(), (trainingArgs).size());
...
bool JasmineGraphServer::initiateClient(std::string host, int port,
                                        int dataPort, std::string trainingArgs,
                                        int iteration, string partCount) {
  Utils utils;
  bool result = true;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    server_logger.log(
        "Sent fed : " + JasmineGraphInstanceProtocol::INITIATE_CLIENT, "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
...
bool JasmineGraphServer::initiateAggregator(std::string host, int port,
                                            int dataPort,
                                            std::string trainingArgs,
                                            int iteration, string partCount) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    server_logger.log("Sent : " + JasmineGraphInstanceProtocol::INITIATE_AGG,
                      "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
...
bool JasmineGraphServer::initiateOrgServer(std::string host, int port,
                                           int dataPort,
                                           std::string trainingArgs,
                                           int iteration, string partCount) {
  Utils utils;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    server_logger.log(
        "Sent fed : " + JasmineGraphInstanceProtocol::INITIATE_ORG_SERVER,
        "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
...
bool JasmineGraphServer::receiveGlobalWeights(std::string host, int port,
                                              std::string trainingArgs,
                                              int iteration,
  ...
  std::string weights_file =
      utils.getJasmineGraphProperty("org.jasminegraph.fl.weights.file");
  std::string flagPath =
      utils.getJasmineGraphProperty("org.jasminegraph.fl.flag.file");
...
bool JasmineGraphServer::mergeFiles(std::string host, int port, int dataPort,
                                    std::string trainingArgs, int iteration,
                                    string partCount) {
  Utils utils;
  bool result = true;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    server_logger.log(
        "Received merge : " + JasmineGraphInstanceProtocol::HANDSHAKE_OK,
        "info");
    string server_host =
        utils.getJasmineGraphProperty("org.jasminegraph.server.host");
    write(sockfd, server_host.c_str(), server_host.size());
    ...
    server_logger.log(
        "Sent merge : " + JasmineGraphInstanceProtocol::MERGE_FILES, "info");
    bzero(data, FED_DATA_LENGTH);
    read(sockfd, data, FED_DATA_LENGTH);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      server_logger.log("Received merge : " + JasmineGraphInstanceProtocol::OK,
                        "info");
      write(sockfd, (trainingArgs).c_str(), (trainingArgs).size());
...
bool JasmineGraphServer::sendTrainCommand(std::string host, int port,
                                          std::string trainingArgs) {
  Utils utils;
  bool result = true;
  int sockfd;
