...
NodeManager::NodeManager(GraphConfig gConfig) {
  ...
  std::string graphPrefix =
      instanceDataFolderLocation + "/g" + std::to_string(graphID);
  ...
  std::ios_base::openmode openMode =
      std::ios::trunc;  // default is Trunc mode which overrides the entire file
  if (gConfig.openMode == NodeManager::FILE_MODE) {
    openMode = std::ios::app;  // if app, open in append mode
    this->nodeIndex = readNodeIndex();
  }
  NodeBlock::nodesDB = new std::fstream(
      nodesDBPath, std::ios::in | std::ios::out | openMode | std::ios::binary);
  PropertyLink::propertiesDB = new std::fstream(
      dbPrefix + "_properties.db",
      std::ios::in | std::ios::out | openMode | std::ios::binary);
  RelationBlock::relationsDB = new std::fstream(
      dbPrefix + "_relations.db",
      std::ios::in | std::ios::out | openMode | std::ios::binary);
  // TODO (tmkasun): set PropertyLink nextPropertyIndex after validating by
  // modulus check from file number of bytes

  if (dbSize(nodesDBPath) % NodeBlock::BLOCK_SIZE != 0) {
    std::string errorMessage =
        "Node DB size does not comply to node block size Path = " +
...
std::unordered_map<std::string, unsigned int> NodeManager::readNodeIndex() {
  ...
  std::unordered_map<std::string, unsigned int>
      _nodeIndex;  // temporary node index data holder
  ...
  if (index_db.is_open()) {
    ...
    if (iSize % dataWidth != 0) {
      node_manager_logger.error(
          "Index DB size does not comply to index block size Path = " +
          this->index_db_loc);
      throw std::runtime_error("Node index DB in " + this->index_db_loc +
                               " is corrupted!");
    }
    ...
    for (size_t i = 0; i < iSize / dataWidth; i++) {
      nodeIDC[NodeManager::INDEX_KEY_SIZE] = {
          0};  // Fill with null chars before putting data
      if (!index_db.read(&nodeIDC[0], NodeManager::INDEX_KEY_SIZE)) {
        node_manager_logger.error(
            "Error while reading index key data from block i = " +
            std::to_string(i));
      }
      if (!index_db.read(reinterpret_cast<char *>(&nodeIndexId),
                         sizeof(unsigned int))) {
        node_manager_logger.error(
            "Error while reading index ID data from block i = " +
            std::to_string(i));
      }
...
RelationBlock *NodeManager::addRelation(NodeBlock source,
                                        NodeBlock destination) {
  RelationBlock *newRelation = NULL;
  if (source.edgeRef == 0 || destination.edgeRef == 0 ||
      !source.searchRelation(
          destination)) {  // certainly a new relation block needed
    ...
    if (newRelation) {
      ...
      node_manager_logger.error(
          "Error while adding the new edge/relation for source = " +
          std::string(source.id) +
          " destination = " + std::string(destination.id));
    }
  } else {
    // TODO[tmkasun]: implement get edge support and return existing
    // edge/relation if already exist
    node_manager_logger.warn(
        "Relation/Edge already exist for source = " + std::string(source.id) +
        " destination = " + std::string(destination.id));
  }
  return newRelation;
}
...
NodeBlock *NodeManager::addNode(std::string nodeId) {
  ...
  node_manager_logger.debug("Adding node index " +
                            std::to_string(this->nextNodeIndex));
  if (this->nodeIndex.find(nodeId) == this->nodeIndex.end()) {
    node_manager_logger.debug("Can't find NodeId (" + nodeId +
                              ") in the index database");
    NodeBlock *sourceBlk =
        new NodeBlock(nodeId, this->nextNodeIndex * NodeBlock::BLOCK_SIZE);
...
RelationBlock *NodeManager::addEdge(std::pair<std::string, std::string> edge) {
  ...
  node_manager_logger.debug(
      "DEBUG: Source DB block address " + std::to_string(sourceNode->addr) +
...
int NodeManager::dbSize(std::string path) {
  ...
  */
  ...
  if (stat(path.c_str(), &result) == 0) {
    node_manager_logger.debug("Size of the " + path + " is " +
                              std::to_string(result.st_size));
  } else {
...
NodeBlock *NodeManager::get(std::string nodeId) {
  ...
  if (!NodeBlock::nodesDB->get(usageBlock)) {
    node_manager_logger.error("Error while reading usage data from block " +
                              std::to_string(blockAddress));
  }

  if (!NodeBlock::nodesDB->read(reinterpret_cast<char *>(&edgeRef),
                                sizeof(unsigned int))) {
    node_manager_logger.error(
        "Error while reading edge reference data from block " +
        std::to_string(blockAddress));
  }

  if (!NodeBlock::nodesDB->read(reinterpret_cast<char *>(&edgeRefPID),
                                sizeof(unsigned char))) {
    node_manager_logger.error("Error while reading usage data from block " +
                              std::to_string(blockAddress));
  }

  if (!NodeBlock::nodesDB->read(reinterpret_cast<char *>(&propRef),
                                sizeof(unsigned int))) {
    node_manager_logger.error(
        "Error while reading prop reference data from block " +
        std::to_string(blockAddress));
  }

  if (!NodeBlock::nodesDB->read(&label[0], NodeBlock::LABEL_SIZE)) {
    node_manager_logger.error("Error while reading label data from block " +
                              std::to_string(blockAddress));
  }
  bool usage = usageBlock == '\1';
  node_manager_logger.debug("Label = " + std::string(label));
  node_manager_logger.debug("Length of label = " +
                            std::to_string(strlen(label)));
  node_manager_logger.debug("DEBUG: raw edgeRef from DB (disk) " +
                            std::to_string(edgeRef));

  nodeBlockPointer = new NodeBlock(nodeId, blockAddress, propRef, edgeRef,
                                   edgeRefPID, label, usage);

  node_manager_logger.debug(
      "DEBUG: nodeBlockPointer after creating the object edgeRef " +
      std::to_string(nodeBlockPointer->edgeRef));

  if (nodeBlockPointer->edgeRef % RelationBlock::BLOCK_SIZE != 0) {
    throw "Exception: Invalid edge reference address = " +
        nodeBlockPointer->edgeRef;
  }
  return nodeBlockPointer;
}

void NodeManager::persistNodeIndex() {
  std::ofstream index_db(this->index_db_loc,
                         std::ios::trunc | std::ios::binary);
  if (index_db.is_open()) {
    if (this->nodeIndex.size() > 0 && (this->nodeIndex.begin()->first.length() >
                                       NodeManager::INDEX_KEY_SIZE)) {
      node_manager_logger.error(
          "Node label/ID is longer ( " +
          std::to_string(this->nodeIndex.begin()->first.length()) +
          " ) than the index key size " +
          std::to_string(NodeManager::INDEX_KEY_SIZE));
      throw "Node label/ID is longer than the index key size!";
    }
    for (auto nodeMap : this->nodeIndex) {
      char nodeIDC[NodeManager::INDEX_KEY_SIZE] = {
          0};  // Initialize with null chars
      std::strcpy(nodeIDC, nodeMap.first.c_str());
      index_db.write(nodeIDC, sizeof(nodeIDC));
      unsigned int nodeBlockIndex = nodeMap.second;
      index_db.write(reinterpret_cast<char *>(&(nodeBlockIndex)),
                     sizeof(unsigned int));
      node_manager_logger.debug(
          "Writing node index --> Node key = " + std::string(nodeIDC) +
 ...
 * It closes all the open databases and persist the node index in-memory hash
 * map to node index database
 *
 * **/
 ...
 * By default Node/Vertext ID or the label is used as the key in the node index
 * database, which act as a lookup table for node IDs to their block address in
 * node DB
 *
 * */
void NodeManager::setIndexKeySize(unsigned long newIndexKeySize) {
  if (NodeBlock::LABEL_SIZE > newIndexKeySize) {
    node_manager_logger.warn(
        "Node/Vertext ID or label is larger than the index key size!!");
  }
...
const std::string NodeManager::FILE_MODE =
    "app";  // for appending to existing DB
