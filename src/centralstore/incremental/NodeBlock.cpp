...
NodeBlock::NodeBlock(std::string id, unsigned int address, unsigned int propRef,
                     unsigned int edgeRef, unsigned char edgeRefPID,
                     char _label[], bool usage)
    : id(id),
      addr(address),
      propRef(propRef),
      edgeRef(edgeRef),
...
void NodeBlock::save() {
  ...
  NodeBlock::nodesDB->write(reinterpret_cast<char *>(&(this->edgeRef)),
                            sizeof(this->edgeRef));
  NodeBlock::nodesDB->put(this->edgeRefPID);
  NodeBlock::nodesDB->write(reinterpret_cast<char *>(&(this->propRef)),
                            sizeof(this->propRef));
    ...
    if (currentHead) {
      if (thisAddress == currentHead->source.address) {
        ...
        throw std::to_string(thisAddress) +
            " relation head does not contain current node in its source or "
            "destination";
      }
      if (thisAddress == newRelation->source.address) {
        ...
        throw std::to_string(thisAddress) +
            " new relation does not contain current node in its source or "
            "destination";
      }
    }
      ...
      if (currentRelation->source.address == this->addr) {
        if (currentRelation->source.nextRelationId == 0) {
          ...
          currentRelation = currentRelation->nextSource();
                 ...
                 currentRelation->destination.address == this->addr) {
...
bool NodeBlock::setRelationHead(RelationBlock newRelation) {
  ...
  if (!NodeBlock::nodesDB->write(
          reinterpret_cast<char *>(&(edgeReferenceAddress)),
          sizeof(unsigned int))) {
    node_block_logger.error(
        "ERROR: Error while updating edge reference address of " +
        std::to_string(edgeReferenceAddress) + " for node " +
...
/**
 * Return a pointer to matching relation block with the given node if found,
 * Else return NULL
 * **/
RelationBlock *NodeBlock::searchRelation(NodeBlock withNode) {
  ...
  while (currentRelation) {
    if (currentRelation->source.address == this->addr) {
      ...
      throw "Exception: Unrelated relation block for " +
          std::to_string(this->addr) + " found in relation block " +
          std::to_string(currentRelation->addr);
    }
  }
...
std::list<NodeBlock> NodeBlock::getEdges() {
  ...
  while (currentRelation != NULL) {
    ...
    if (currentRelation->source.address == this->addr) {
      ...
      throw "Error: Unrecognized relation for " + std::to_string(this->addr) +
...
NodeBlock *NodeBlock::get(unsigned int blockAddress) {
  ...
  if (!NodeBlock::nodesDB->get(usageBlock)) {
    node_block_logger.error("Error while reading usage data from block " +
  ...
  if (!NodeBlock::nodesDB->read(reinterpret_cast<char *>(&edgeRefPID),
    ...
    node_block_logger.error(
        "Error while reading edge reference partition ID data from block " +
        std::to_string(blockAddress));
  }
  ...
  if (!NodeBlock::nodesDB->read(&label[0], NodeBlock::LABEL_SIZE)) {
    node_block_logger.error("Error while reading label data from block " +
                            std::to_string(blockAddress));
  }
  ...
  nodeBlockPointer = new NodeBlock(id, blockAddress, propRef, edgeRef,
                                   edgeRefPID, label, usage);
  if (nodeBlockPointer->id.length() ==
      0) {  // if label not found in node block look in the properties
  ...
  node_block_logger.debug("Edge ref = " +
...
PropertyLink *NodeBlock::getPropertyHead() {
  return PropertyLink::get(this->propRef);
}
