...
PropertyLink::PropertyLink(unsigned int propertyBlockAddress)
    : blockAddress(propertyBlockAddress) {
  if (propertyBlockAddress > 0) {
    this->propertiesDB->seekg(propertyBlockAddress);
    char rawName[PropertyLink::MAX_NAME_SIZE] = {0};

    if (!this->propertiesDB->read(reinterpret_cast<char *>(&rawName),
                                  PropertyLink::MAX_NAME_SIZE)) {
      property_link_logger.error(
          "Error while reading property name from block " +
          std::to_string(blockAddress));
    }
    property_link_logger.debug(
        "Current file descriptor curser position = " +
        std::to_string(this->propertiesDB->tellg()) +
        " when reading = " + std::to_string(blockAddress));
    if (!this->propertiesDB->read(reinterpret_cast<char *>(&this->value),
                                  PropertyLink::MAX_VALUE_SIZE)) {
      property_link_logger.error(
          "Error while reading property value from block " +
          std::to_string(blockAddress));
    }

    if (!this->propertiesDB->read(
            reinterpret_cast<char *>(&(this->nextPropAddress)),
            sizeof(unsigned int))) {
      property_link_logger.error(
          "Error while reading property next address from block " +
          std::to_string(blockAddress));
    }
...
PropertyLink::PropertyLink(unsigned int blockAddress, std::string name,
 ...
 * ### Note: This link list implementation adds a new link to the tail of the
 * link, unlike in normal case relocating the head link
 *
 * propertyAddress; If no property address means no property has been stored
 * if(!propertyAddress) { // Means this is the very first link in the chain yet
 * to be filled store property in this link it self } else if ( //Check this
 * property name is available in this link) {
 ...
 *      } else { // If no next, That means this is leaf or at the end of the
 * link, So add a new link and update the current link's next property address
 *          // attach new link
 *      }
 *  }
 * if propertyAddress was 0 before inserting you will get head address of the
 * link in return value else either updated link address or last appended link
...
unsigned int PropertyLink::insert(std::string name, char *value) {
  ...
  std::memcpy(
      dataValue, value,
      PropertyLink::MAX_VALUE_SIZE);  // strcpy or strncpy get terminated at
                                      // null-character hence using memcpy
  ...
  if (this->name == name) {
    ...
    property_link_logger.warn("Property key/name already exist key = " +
                              std::string(name));
    ...
    property_link_logger.debug("Next prop index = " +
                               std::to_string(PropertyLink::nextPropertyIndex));

    unsigned int newAddress =
        PropertyLink::nextPropertyIndex * PropertyLink::PROPERTY_BLOCK_SIZE;
    this->propertiesDB->seekp(newAddress);
    this->propertiesDB->write(dataName, PropertyLink::MAX_NAME_SIZE);
    this->propertiesDB->write(reinterpret_cast<char *>(dataValue),
                              PropertyLink::MAX_VALUE_SIZE);
    if (!this->propertiesDB->write(reinterpret_cast<char *>(&nextAddress),
                                   sizeof(nextAddress))) {
      property_link_logger.error("Error while inserting a property " + name +
                                 " into block address " +
    ...
    this->propertiesDB->seekp(
        this->blockAddress + PropertyLink::MAX_NAME_SIZE +
        PropertyLink::MAX_VALUE_SIZE);  // seek to current property next address
    if (!this->propertiesDB->write(reinterpret_cast<char *>(&newAddress),
                                   sizeof(newAddress))) {
      property_link_logger.error(
          "Error while updating  property next address for " + name +
          " into block address " + std::to_string(this->blockAddress));
      return -1;
    }

    PropertyLink::nextPropertyIndex++;  // Increment the shared property index
                                        // value
    return newAddress;
  }
}
...
PropertyLink *PropertyLink::create(std::string name, char value[]) {
  ...
  PropertyLink::propertiesDB->write(reinterpret_cast<char *>(value),
                                    PropertyLink::MAX_VALUE_SIZE);
  if (!PropertyLink::propertiesDB->write(reinterpret_cast<char *>(&nextAddress),
                                         sizeof(nextAddress))) {
    property_link_logger.error(
        "Error while inserting the property = " + name +
...
/**
 * If a nextPropAddress available return PropertyLink with that else return null
 * pointer
 * */
...
PropertyLink *PropertyLink::get(unsigned int propertyBlockAddress) {
  ...
  if (propertyBlockAddress > 0) {
    ...
    if (!PropertyLink::propertiesDB->read(
            reinterpret_cast<char *>(&propertyName),
            PropertyLink::MAX_NAME_SIZE)) {
      property_link_logger.error(
          "Error while reading property name from block = " +
    ...
    if (!PropertyLink::propertiesDB->read(
            reinterpret_cast<char *>(&propertyValue),
            PropertyLink::MAX_VALUE_SIZE)) {
      property_link_logger.error(
          "Error while reading property value from block = " +
          std::to_string(propertyBlockAddress));
    ...
    if (!PropertyLink::propertiesDB->read(
            reinterpret_cast<char *>(&(nextAddress)), sizeof(unsigned int))) {
      property_link_logger.error(
          "Error while reading property next address from block = " +
          std::to_string(propertyBlockAddress));
    }
    pl = new PropertyLink(propertyBlockAddress, std::string(propertyName),
                          propertyValue, nextAddress);
  }
  return pl;
}
unsigned int PropertyLink::nextPropertyIndex =
