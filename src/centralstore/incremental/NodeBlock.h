...
class NodeBlock {
 ...
 public:
  unsigned int addr = 0;
  std::string id = "";       // Node ID for this block ie: citation paper ID,
                             // Facebook accout ID, Twitter account ID etc
  ...
  char label[LABEL_SIZE] = {
      0};  // Initialize with null chars label === ID if length(id) < 6 else ID
           // will be stored as a Node's property
  ...
  NodeBlock(std::string id, unsigned int address, unsigned int propRef,
            unsigned int edgeRef, unsigned char edgeRefPID, char _label[],
            bool usage);
