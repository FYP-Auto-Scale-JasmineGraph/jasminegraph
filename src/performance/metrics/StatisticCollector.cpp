...
std::string StatisticCollector::collectVMStatistics(
    std::string isVMStatManager, std::string isTotalAllocationRequired) {
  ...
  if (isVMStatManager == "true") {
    ...
    vmLevelStatistics =
        std::to_string(totalMemoryUsed) + "," + cpuUsageString + ",";
  }
  ...
  if (isTotalAllocationRequired == "true") {
    ...
    vmLevelStatistics = vmLevelStatistics + std::to_string(totalMemory) + "," +
                        std::to_string(totalCoresAvailable);
  }
...
double StatisticCollector::getTotalCpuUsage() {
  ...
  if (input) {
    ...
    if (!result.empty()) {
      ...
      std::vector<std::string> splittedParamNames =
          utils.split(parameterNames, ' ');
      splittedParamNames.erase(
          std::remove(splittedParamNames.begin(), splittedParamNames.end(), ""),
          splittedParamNames.end());
      std::vector<std::string> splittedParamValues =
          utils.split(parameterValues, ' ');
      splittedParamValues.erase(std::remove(splittedParamValues.begin(),
                                            splittedParamValues.end(), ""),
                                splittedParamValues.end());

      for (paramNameIterator = splittedParamNames.begin();
           paramNameIterator != splittedParamNames.end(); ++paramNameIterator) {
        std::string paramName = *paramNameIterator;

        if (paramName.find("%") != std::string::npos &&
            paramName.find("idle") == std::string::npos) {
