...
int PerformanceUtil::collectPerformanceStatistics() {
  ...
  std::string placeLoadQuery =
      "select ip, user, server_port, is_master, "
      "is_host_reporter,host_idhost,idplace from place";
  std::vector<vector<pair<string, string>>> placeList =
      perfDb.runSelect(placeLoadQuery);
  ...
  for (placeListIterator = placeList.begin();
    ...
    if (isHostReporter.find("true") != std::string::npos) {
      std::string hostSearch =
          "select total_cpu_cores,total_memory from host where idhost='" +
          hostId + "'";
      std::vector<vector<pair<string, string>>> hostAllocationList =
          perfDb.runSelect(hostSearch);

      vector<pair<string, string>> firstHostAllocation =
          hostAllocationList.at(0);
    ...
    if (isMaster.find("true") != std::string::npos) {
      collectLocalPerformanceData(isHostReporter, requestResourceAllocation,
                                  hostId, placeId);
    } else {
      collectRemotePerformanceData(host, atoi(serverPort.c_str()),
                                   isHostReporter, requestResourceAllocation,
                                   hostId, placeId);
    }
  }
...
int PerformanceUtil::collectSLAResourceConsumption(std::vector<Place> placeList,
                                                   std::string graphId,
                                                   std::string masterIP,
                                                   int elapsedTime) {
...
std::vector<ResourceConsumption>
PerformanceUtil::retrieveCurrentResourceUtilization(std::string masterIP) {
  std::string placeLoadQuery =
      "select ip, user, server_port, is_master, "
      "is_host_reporter,host_idhost,idplace from place";
  std::vector<vector<pair<string, string>>> placeList =
  ...
  for (placeListIterator = placeList.begin();
    ...
    if ((isMaster.find("true") != std::string::npos || host == "localhost" ||
      ...
      resourceConsumption = retrieveRemoteResourceConsumption(
          host, atoi(serverPort.c_str()), hostId, placeId);
      placeResourceConsumptionList.push_back(resourceConsumption);
    }
  }
...
void PerformanceUtil::collectRemotePerformanceData(
    std::string host, int port, std::string isVMStatManager,
  ...
  scheduler_logger.log("Sent : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                       "info");
  bzero(data, 301);
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    scheduler_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::PERFORMANCE_STATISTICS,
        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    scheduler_logger.log("Received : " + JasmineGraphInstanceProtocol::OK,
                         "info");
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      scheduler_logger.log("Sent : VM Manager Status " + isVMStatManager,
                           "info");
      ...
      if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
        write(sockfd, isResourceAllocationRequired.c_str(),
              isResourceAllocationRequired.size());
        scheduler_logger.log("Sent : Resource Allocation Requested " +
        ...
        if (isVMStatManager == "true" && strArr.size() > 4) {
          ...
          string vmPerformanceSql =
              "insert into host_performance_data (date_time, memory_usage, "
              "cpu_usage, idhost) values ('" +
              processTime + "','" + memoryConsumption + "','" + cpuUsage +
              "','" + hostId + "')";
          ...
          if (isResourceAllocationRequired == "true") {
            ...
            string allocationUpdateSql = "update host set total_cpu_cores='" +
                                         totalCores + "',total_memory='" +
                                         totalMemory + "' where idhost='" +
                                         hostId + "'";
        ...
        string placePerfSql =
            "insert into place_performance_data (idplace, memory_usage, "
            "cpu_usage, date_time) values ('" +
            placeId + "','" + memoryUsage + "','" + cpuUsage + "','" +
            processTime + "')";
...
void PerformanceUtil::collectLocalPerformanceData(
    std::string isVMStatManager, std::string isResourceAllocationRequired,
  ...
  if (isVMStatManager.find("true") != std::string::npos) {
    ...
    string vmPerformanceSql =
        "insert into host_performance_data (date_time, memory_usage, "
        "cpu_usage, idhost) values ('" +
        reportTimeString + "','" + totalMemoryUsed + "','" + totalCPUUsage +
        "','" + hostId + "')";
    ...
    if (isResourceAllocationRequired == "true") {
      ...
      string allocationUpdateSql =
          "update host set total_cpu_cores='" + totalCores +
  ...
  string placePerfSql =
      "insert into place_performance_data (idplace, memory_usage, cpu_usage, "
      "date_time) values ('" +
...
int PerformanceUtil::collectRemoteSLAResourceUtilization(
    std::string host, int port, std::string isVMStatManager,
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
      write(sockfd,
            JasmineGraphInstanceProtocol::PERFORMANCE_STATISTICS.c_str(),
            JasmineGraphInstanceProtocol::PERFORMANCE_STATISTICS.size());
      scheduler_logger.log(
          "Sent : " + JasmineGraphInstanceProtocol::PERFORMANCE_STATISTICS,
          "info");
...
void PerformanceUtil::collectLocalSLAResourceUtilization(std::string placeId,
                                                         int elapsedTime) {
  StatisticCollector statisticCollector;
  Utils utils;
  statisticCollector.init();
...
ResourceConsumption PerformanceUtil::retrieveLocalResourceConsumption(
    std::string host, std::string placeId) {
...
ResourceConsumption PerformanceUtil::retrieveRemoteResourceConsumption(
    std::string host, int port, std::string hostId, std::string placeId) {
  ResourceConsumption placeResourceConsumption;
...
std::vector<long> PerformanceUtil::getResourceAvailableTime(
    std::vector<std::string> graphIdList, std::string command,
  ...
  for (processInfoIterator = processData.begin();
       processInfoIterator != processData.end(); ++processInfoIterator) {
    ...
    if (process.priority == Conts::HIGH_PRIORITY_DEFAULT_VALUE) {
      ...
      long elapsedTime =
          currentTimestamp - (processScheduledTime + initialSleepTime);
      ...
      if (elapsedTime < 0) {
        ...
        long jobIdleTime =
            (processScheduledTime + initialSleepTime) - currentTimestamp;
      ...
      std::string slaLoadQuery =
          "select "
          "graph_place_sla_performance.place_id,graph_place_sla_performance."
          "load_average,"
          "graph_place_sla_performance.elapsed_time from "
          "graph_place_sla_performance inner join graph_sla"
          "  inner join sla_category where "
          "graph_place_sla_performance.graph_sla_id=graph_sla.id"
          "  and graph_sla.id_sla_category=sla_category.id and "
          "graph_sla.graph_id='" +
          currentGraphId + "' and sla_category.command='" + command +
          "' and sla_category.category='" + category +
          "' and graph_place_sla_performance.elapsed_time > '" +
          std::to_string(adjustedElapsedTime) +
          "' order by "
          "graph_place_sla_performance.place_id,graph_place_sla_performance."
          "elapsed_time;";

      std::vector<vector<pair<string, string>>> loadAvgResults =
          perfDb.runSelect(slaLoadQuery);

      std::map<std::string, std::vector<double>> hostTmpLoadAvgMap;

      for (std::vector<vector<pair<string, string>>>::iterator i =
               loadAvgResults.begin();
      ...
      for (std::map<std::string, std::vector<double>>::iterator mapIterator =
               hostTmpLoadAvgMap.begin();
        ...
        for (std::size_t index = 0; index != hostLoadVector.size() - 1;
             ++index) {
          ...
          double adjustedLoad =
              slope * (xAxisValue + requiredAdjustment) + (intercept);
        ...
        std::vector<double> existingAggregateLoadVector =
            hostLoadAvgMap[placeId];

        if (existingAggregateLoadVector.size() > 0) {
          auto maxVectorSize = std::max(existingAggregateLoadVector.size(),
                                        adjustedHostLoads.size());
          std::vector<double> aggregatedVector =
              std::vector<double>(maxVectorSize);

          existingAggregateLoadVector.resize(maxVectorSize);
          adjustedHostLoads.resize(maxVectorSize);

          std::transform(existingAggregateLoadVector.begin(),
                         existingAggregateLoadVector.end(),
                         adjustedHostLoads.begin(), aggregatedVector.begin(),
                         std::plus<double>());
  ...
  for (graphIdListIterator = graphIdList.begin();
       graphIdListIterator != graphIdList.end(); ++graphIdListIterator) {
    std::string graphId = *graphIdListIterator;
    scheduler_logger.log("###PERFORMANCE### New Job Graph ID:" + graphId,
                         "info");
    int loopCount = 0;

    std::string newJobLoadQuery =
        "select "
        "graph_place_sla_performance.place_id,graph_place_sla_performance.load_"
        "average,"
        "graph_place_sla_performance.elapsed_time from "
        "graph_place_sla_performance inner join graph_sla"
        "  inner join sla_category where "
        ...
        "  and graph_sla.id_sla_category=sla_category.id and "
        "graph_sla.graph_id='" +
        graphId + "' and sla_category.command='" + command +
        "' and sla_category.category='" + category +
        "' order by "
        "graph_place_sla_performance.place_id,graph_place_sla_performance."
        "elapsed_time;";

    std::vector<vector<pair<string, string>>> newJobLoadAvgResults =
        perfDb.runSelect(newJobLoadQuery);

    std::map<std::string, std::vector<double>> newJobLoadAvgMap;

    for (std::vector<vector<pair<string, string>>>::iterator i =
             newJobLoadAvgResults.begin();
    ...
    for (std::map<std::string, std::vector<double>>::iterator mapIterator =
      ...
      if (newJobHostLoadVector.size() > 0) {
        double newJobHostMaxLoad = *std::max_element(
            newJobHostLoadVector.begin(), newJobHostLoadVector.end());
        double currentAggHostMaxLoad =
            *std::max_element(hostLoadVector.begin(), hostLoadVector.end());

        scheduler_logger.log("###PERFORMANCE### New Job Max Load:" +
                                 std::to_string(newJobHostMaxLoad),
                             "info");
        scheduler_logger.log("###PERFORMANCE### Current Aggregated max load:" +
                                 std::to_string(currentAggHostMaxLoad),
                             "info");

        if (Conts::LOAD_AVG_THREASHOLD >
            currentAggHostMaxLoad + newJobHostMaxLoad) {
          jobAcceptableTime = 0;
        } else {
          int maxValueIndex =
              std::max_element(hostLoadVector.begin(), hostLoadVector.end()) -
              hostLoadVector.begin();
          bool flag = true;

          for (int aggIndex = maxValueIndex; aggIndex != hostLoadVector.size();
               ++aggIndex) {
            for (int currentJobIndex = 0;
                 currentJobIndex != newJobHostLoadVector.size();
                 ++currentJobIndex) {
              if (aggIndex + currentJobIndex >= hostLoadVector.size()) {
                break;
              }

              if (hostLoadVector[aggIndex + currentJobIndex] +
                      newJobHostLoadVector[currentJobIndex] >
            ...
            if (flag && aggIndex * Conts::LOAD_AVG_COLLECTING_GAP * 1000 >
                            jobAcceptableTime) {
              jobAcceptableTime =
                  aggIndex * Conts::LOAD_AVG_COLLECTING_GAP * 1000;
            } else {
              flag = true;
            }
          }
    ...
    scheduler_logger.log("###PERFORMANCE### New Job onboard after:" +
                             std::to_string(jobAcceptableTime),
                         "info");

    JobRequest jobRequest = pendingHPJobList[loopCount];
    std::string slaString =
        jobRequest.getParameter(Conts::PARAM_KEYS::GRAPH_SLA);
    long graphSla = std::atol(slaString.c_str());

    if (jobAcceptableTime > graphSla * 0.1) {
      jobScheduleVector.push_back(-1);
    } else {
      PerformanceUtil::adjustAggregateLoadMap(hostLoadAvgMap, newJobLoadAvgMap,
                                              jobAcceptableTime);
  ...
  for (logIterator = hostLoadAvgMap.begin();
       logIterator != hostLoadAvgMap.end(); ++logIterator) {
    ...
    for (loadIterator = loadVector.begin(); loadIterator != loadVector.end();
         ++loadIterator) {
      double load = *loadIterator;
      std::cout << "###PERF### Host ID: " + hostId +
                       "EXPECTED LOAD: " + std::to_string(load)
                << std::endl;
    }
  }
...
void PerformanceUtil::adjustAggregateLoadMap(
    std::map<std::string, std::vector<double>> &aggregateLoadAvgMap,
    std::map<std::string, std::vector<double>> &newJobLoadAvgMap,
    long newJobAcceptanceTime) {
  std::map<std::string, std::vector<double>>::iterator newJobIterator;

  for (newJobIterator = newJobLoadAvgMap.begin();
       newJobIterator != newJobLoadAvgMap.end(); ++newJobIterator) {
    ...
    if (newJobAcceptanceTime > 0) {
      for (long adjustCount = 0; adjustCount < newJobAcceptanceTime;
        ...
        newJobTmpLoadVector.insert(newJobTmpLoadVector.end(),
                                   newJobLoadVector.begin(),
    ...
    if (aggregateLoadVector.size() > 0) {
      auto maxVectorSize =
          std::max(newJobTmpLoadVector.size(), aggregateLoadVector.size());
      ...
      std::transform(aggregateLoadVector.begin(), aggregateLoadVector.end(),
                     newJobTmpLoadVector.begin(), aggregatedVector.begin(),
                     std::plus<double>());
...
void PerformanceUtil::logLoadAverage() {
  ...
  std::cout << "###PERF### CURRENT LOAD: " + std::to_string(currentLoadAverage)
            << std::endl;
}

void PerformanceUtil::updateResourceConsumption(
    PerformanceSQLiteDBInterface performanceDb, std::string graphId,
  ...
  for (placeListIterator = placeList.begin();
    ...
    std::string query = "SELECT id from graph_sla where graph_id='" + graphId +
                        ...
                        "' and id_sla_category='" + slaCategoryId + "';";

    std::vector<vector<pair<string, string>>> results =
        performanceDb.runSelect(query);

    if (results.size() == 1) {
      graphSlaId = results[0][0].second;
    } else {
      std::string insertQuery =
          "insert into graph_sla (id_sla_category, graph_id, partition_count, "
          "sla_value, attempt) VALUES ('" +
          slaCategoryId + "','" + graphId + "'," +
          std::to_string(partitionCount) + ",0,0);";
    ...
    std::vector<ResourceUsageInfo> resourceUsageVector =
        resourceUsageMap[placeId];
    string valuesString;
    std::vector<ResourceUsageInfo>::iterator usageIterator;

    if (resourceUsageVector.size() > 0) {
      string slaPerfSql =
          "insert into graph_place_sla_performance (graph_sla_id, place_id, "
          "memory_usage, load_average, elapsed_time) "
          "values ";

      for (usageIterator = resourceUsageVector.begin();
           usageIterator != resourceUsageVector.end(); ++usageIterator) {
...
void PerformanceUtil::updateRemoteResourceConsumption(
    PerformanceSQLiteDBInterface performanceDb, std::string graphId,
  ...
  for (placeListIterator = placeList.begin();
    ...
    if (isMaster.find("true") != std::string::npos || host == "localhost" ||
      ...
      std::string loadString = requestRemoteLoadAverages(
          host, atoi(serverPort.c_str()), isHostReporter, "false", placeId, 0,
          masterIP);
      std::vector<std::string> loadVector = Utils::split(loadString, ',');
      std::vector<std::string>::iterator loadVectorIterator;
      string valuesString;
      std::string query = "SELECT id from graph_sla where graph_id='" +
                          graphId + "' and partition_count='" +
                          std::to_string(partitionCount) +
                          "' and id_sla_category='" + slaCategoryId + "';";
      ...
      if (results.size() == 1) {
        ...
        std::string insertQuery =
            "insert into graph_sla (id_sla_category, graph_id, "
            "partition_count, sla_value, attempt) VALUES ('" +
            slaCategoryId + "','" + graphId + "'," +
            std::to_string(partitionCount) + ",0,0);";
      ...
      if (loadVector.size() > 0) {
        ...
        for (loadVectorIterator = loadVector.begin();
             loadVectorIterator != loadVector.end(); ++loadVectorIterator) {
...
std::string PerformanceUtil::getSLACategoryId(std::string command,
                                              std::string category) {
  std::string categoryQuery = "SELECT id from sla_category where command='" +
                              command + "' and category='" + category + "'";

  std::vector<vector<pair<string, string>>> categoryResults =
      perfDb.runSelect(categoryQuery);
  ...
  if (categoryResults.size() == 1) {
    ...
    scheduler_logger.log(
        "Invalid SLA " + category + " for " + command + " command", "error");
    return 0;
  }
}

void PerformanceUtil::initiateCollectingRemoteSLAResourceUtilization(
    std::string host, int port, std::string isVMStatManager,
    std::string isResourceAllocationRequired, std::string placeId,
    int elapsedTime, std::string masterIP) {
  int sockfd;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
      ...
      scheduler_logger.log(
          "Sent : " + JasmineGraphInstanceProtocol::START_STAT_COLLECTION,
          "info");
      bzero(data, 301);
      read(sockfd, data, 300);
      response = (data);
...
std::string PerformanceUtil::requestRemoteLoadAverages(
    std::string host, int port, std::string isVMStatManager,
    std::string isResourceAllocationRequired, std::string placeId,
    int elapsedTime, std::string masterIP) {
  int sockfd;
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    if (response.compare(JasmineGraphInstanceProtocol::HOST_OK) == 0) {
      write(sockfd,
            JasmineGraphInstanceProtocol::REQUEST_COLLECTED_STATS.c_str(),
            JasmineGraphInstanceProtocol::REQUEST_COLLECTED_STATS.size());
      scheduler_logger.log(
          "Sent : " + JasmineGraphInstanceProtocol::REQUEST_COLLECTED_STATS,
          "info");
      bzero(data, 301);
      read(sockfd, data, 300);
      response = (data);
