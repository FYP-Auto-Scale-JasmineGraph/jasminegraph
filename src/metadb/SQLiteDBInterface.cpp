...
int SQLiteDBInterface::init() {
  Utils utils;
  int rc = sqlite3_open(
      utils.getJasmineGraphProperty("org.jasminegraph.db.location").c_str(),
      &database);
  if (rc) {
    db_logger.log("Cannot open database: " + string(sqlite3_errmsg(database)),
                  "error");
    return (-1);
  } else {
...
vector<vector<pair<string, string>>> SQLiteDBInterface::runSelect(
    std::string query) {
  char *zErrMsg = 0;
  int rc;
...
// This function inserts a new row to the DB and returns the last inserted row
// id returns -1 on error
int SQLiteDBInterface::runInsert(std::string query) {
  ...
  if (rc != SQLITE_OK) {
    ...
    int rc2 =
        sqlite3_exec(database, q2.c_str(), callback, &dbResults, &zErrMsg);
