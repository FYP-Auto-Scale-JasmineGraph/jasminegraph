 ...
 *
 * **/
partitionedEdge Partitioner::ldgPartitioning(
    std::pair<std::string, std::string> edge) {
  std::vector<double> partitionScoresFirst(numberOfPartitions,
                                           0);  // Calculate per incoming edge
  std::vector<double> partitionScoresSecond(numberOfPartitions,
                                            0);  // Calculate per incoming edge
  ...
  for (auto partition : partitions) {
    ...
    std::set<std::string> firstVertextNeighbors =
        partition.getNeighbors(edge.first);
    std::set<std::string> secondVertextNeighbors =
        partition.getNeighbors(edge.second);
    double weightedGreedy =
        (1 - (partitionSize / ((double)this->totalVertices /
                               (double)this->numberOfPartitions)));
    ...
    if (firstVertextNeighbors.size() != 0) {
      if (firstVertextNeighbors.find(edge.second) !=
          firstVertextNeighbors.end())
        return {{edge.first, id},
                {edge.second, id}};  // Nothing to do, edge already exisit
    }

    partitionScoresFirst[id] = firstVertextInterCost * weightedGreedy;

    if (secondVertextNeighbors.size() != 0) {
      if (secondVertextNeighbors.find(edge.second) !=
          secondVertextNeighbors.end())
        return {{edge.first, id},
                {edge.second, id}};  // Nothing to do, edge already exisit,
                                     // Because of the symmetrical nature of
  ...
  int firstIndex = distance(
      partitionScoresFirst.begin(),
      max_element(partitionScoresFirst.begin(), partitionScoresFirst.end()));

  int secondIndex = distance(
      partitionScoresSecond.begin(),
...
partitionedEdge Partitioner::hashPartitioning(
    std::pair<std::string, std::string> edge) {
  int firstIndex = std::hash<std::string>()(edge.first) %
                   this->numberOfPartitions;  // Hash partitioning
  int secondIndex = std::hash<std::string>()(edge.second) %
                    this->numberOfPartitions;  // Hash partitioning

  if (firstIndex == secondIndex) {
    this->partitions[firstIndex].addEdge(edge);
  } else {
    this->partitions[firstIndex].addToEdgeCuts(edge.first, edge.second,
                                               secondIndex);
    this->partitions[secondIndex].addToEdgeCuts(edge.second, edge.first,
                                                firstIndex);
  }
  return {{edge.first, firstIndex}, {edge.second, secondIndex}};
}
...
void Partitioner::printStats() {
  ...
  for (auto partition : this->partitions) {
    ...
    streaming_partitioner_logger.info(
        std::to_string(id) + " => Edges count = " + std::to_string(edgesCount));
    streaming_partitioner_logger.info(
        std::to_string(id) +
        " => Edge cuts count = " + std::to_string(edgeCutsCount));
    streaming_partitioner_logger.info(
        std::to_string(id) + " => Cut ratio = " + std::to_string(edgeCutRatio));
    id++;
  }
}
...
and balancing of the partition sizes. Assign the vertext to partition P that
maximize the partition score
 * |N(v) ∩ Si| + ∂c(|Si|)
 * |N(v) ∩ Si| =  number of neighbours of vertex v that are assigned to
partition Si.
 * ∂c(|Si|) = the marginal cost of increasing the partition i by one additional
vertex.
 *
 * Intra-partition cost function : c(x) = αx^γ, for α > 0 and γ ≥ 1 where x =
vertex cardinality
 *   α = m*(k^γ-1) / (n^γ)
 *   Total number of vertices and edges in the graph denoted as |V| = n and |E|
= m.
 *   k is number of partitions
 **/
partitionedEdge Partitioner::fennelPartitioning(
    std::pair<std::string, std::string> edge) {
  std::vector<double> partitionScoresFirst(numberOfPartitions,
                                           0);  // Calculate per incoming edge
  std::vector<double> partitionScoresSecond(numberOfPartitions,
  ...
  const double alpha = this->totalEdges *
                       pow(this->numberOfPartitions, (gamma - 1)) /
                       pow(this->totalVertices, gamma);
  bool firstVertextAlreadyExist(false);
  bool secondVertextAlreadyExist(false);
  ...
  for (auto &partition : partitions) {
    ...
    firstVertextIntraCost =
        alpha * (pow(partitionSize + 1, gamma) - pow(partitionSize, gamma));
