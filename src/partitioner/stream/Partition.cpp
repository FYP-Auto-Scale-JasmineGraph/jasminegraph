...
// The number of edges, the cardinality of E, is called the size of graph and
// denoted by |E|. We usually use m to denote the size of G.
...
// The number of vertices, the cardinality of V, is called the order of graph
// and devoted by |V|. We usually use n to denote the order of G.
double Partition::getVertextCount() {
  ...
  for (size_t i = 0; i < this->numberOfPartitions; i++) {
    for (auto edge : this->edgeCuts[i]) {
      bool isExistInEdgeList =
          this->edgeList.find(edge.first) != this->edgeList.end();
...
void Partition::addToEdgeCuts(std::string resident, std::string foreign,
                              int partitionId) {
  if (partitionId < this->numberOfPartitions) {
    ...
    if (exsistResidentVertex != this->edgeCuts[partitionId].end()) {
      ...
      this->edgeCuts[partitionId][resident] =
...
float Partition::edgeCutsRatio() {
  return this->edgeCutsCount() /
         (this->getEdgesCount() + this->edgeCutsCount());
}

void Partition::printEdgeCuts() {
  streaming_partition_logger.debug("Printing edge cuts of " +
                                   std::to_string(id) + " partition");

  for (auto partition : this->edgeCuts) {
    for (auto edgeList : partition) {
      streaming_partition_logger.debug("edgeList.first " + edgeList.first +
                                       " ____");
...
void Partition::printEdges() {
  streaming_partition_logger.debug("Printing edge list of " +
                                   std::to_string(id) + " partition");
  ...
  for (auto edgeList : this->edgeList) {
    ...
    for (std::string vertext : edgeList.second) {
      ...
      if (compositeVertextIDs.find(compositeVertextID) ==
          compositeVertextIDs.end()) {
        if (isFirst) {
          streaming_partition_logger.debug("edgeList.first " + edgeList.first +
                                           " ____");
          isFirst = false;
