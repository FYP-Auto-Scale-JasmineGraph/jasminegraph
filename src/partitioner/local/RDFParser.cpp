...
GetConfig::GetConfig() {
  ...
  TAG_Book_Section_Reference =
      XMLString::transcode("akt:Book-Section-Reference");
  TAG_Thesis_Reference = XMLString::transcode("akt:Thesis-Reference");
  TAG_Book_Reference = XMLString::transcode("akt:Book-Reference");
  TAG_Conference_Proceedings_Reference =
      XMLString::transcode("akt:Conference-Proceedings-Reference");
...
GetConfig::~GetConfig() {
  ...
  try {
    ...
    cerr << "Unknown exception encountered while trying to release unknown TAG"
         << endl;
  }
          ...
          currentNode->getNodeType() == DOMNode::ELEMENT_NODE)  // is element
      {
        ...
        if (XMLString::equals(currentElement->getTagName(),
          ...
          for (XMLSize_t xx2 = 0; xx2 < nodeCount2; ++xx2) {
                    ...
                    DOMNode::ELEMENT_NODE)  // is element
            {
              ...
              } else if (XMLString::equals(currentElement2->getTagName(),
                                           TAG_has_title)) {
                ...
                for (XMLSize_t e = 0; e < elementCount; ++e) {
                          ...
                          DOMNode::ELEMENT_NODE)  // is element
                  {
                    ...
                    if (XMLString::equals(currentDateElement->getTagName(),
                                          TAG_year_of)) {
                      const XMLCh *yearOf =
                          currentDateElement->getTextContent();
                      year = XMLString::transcode(yearOf);
                      attributes[5] = year;

                    } else if (XMLString::equals(
                                   currentDateElement->getTagName(),
                                   TAG_month_of)) {
                      const XMLCh *monthOf =
                          currentDateElement->getTextContent();
        ...
        for (int i = 0; i < authorsInArticle.size(); i++) {
          for (int j = i + 1; j < authorsInArticle.size(); j++) {
            edgelist.push_back(
                make_pair(authorsInArticle.at(i), authorsInArticle.at(j)));
            addToEdges(&edgeMap, authorsInArticle.at(i), authorsInArticle.at(j),
                       articleID);
...
void GetConfig::writeEdgesToFile() {
  ...
  this->utils.createDirectory(utils.getHomeDir() + "/.jasminegraph/tmp/" +
                              to_string(this->graphID));
  file.open(utils.getHomeDir() + "/.jasminegraph/tmp/" +
...
void GetConfig::addToEdges(std::map<pair<int, int>, int> *map, long node_1,
                           long node_2, long article_id) {
