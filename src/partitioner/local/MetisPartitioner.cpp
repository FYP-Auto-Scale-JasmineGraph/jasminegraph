...
void MetisPartitioner::loadDataSet(string inputFilePath, int graphID) {
  ...
  // Output directory is created under the users home directory
  // '~/.jasminegraph/tmp/'
  this->outputFilePath = utils.getHomeDir() + "/.jasminegraph/tmp/" +
                         std::to_string(this->graphID);

  // Have to call createDirectory twice since it does not support recursive
  // directory creation. Could use boost::filesystem for path creation
  ...
  while (!line.empty()) {
    ...
    if (firstEdgeSet.empty()) {
      ...
      if (std::find(firstEdgeSet.begin(), firstEdgeSet.end(), secondVertex) ==
          firstEdgeSet.end()) {
        firstEdgeSet.push_back(secondVertex);
        edgeCountForMetis++;
      }
    ...
    if (secondEdgeSet.empty()) {
      ...
      if (std::find(secondEdgeSet.begin(), secondEdgeSet.end(), firstVertex) ==
          secondEdgeSet.end()) {
        secondEdgeSet.push_back(firstVertex);
      }
    }
    ...
    std::getline(dbFile, line);
    while (!line.empty() &&
           line.find_first_not_of(splitter) == std::string::npos) {
      std::getline(dbFile, line);
    }
  }
...
int MetisPartitioner::constructMetisFormat(string graph_type) {
  ...
  for (int vertexNum = 0; vertexNum <= largestVertex; vertexNum++) {
    ...
    if (vertexNum > smallestVertex && vertexSet.empty()) {
      partitioner_logger.log(
          "Vertex list is not sequential. Reformatting vertex list", "info");
      vertexCount = 0;
      edgeCount = 0;
      edgeCountForMetis = 0;
      graphEdgeMap.clear();
    ...
    // To handle situations where a blank line gets printed because vertexSet of
    // zero vertex is zero in graphs with no zero vertex
    if (!zeroflag && vertexNum == 0) {
      continue;
    }

    for (std::vector<int>::const_iterator i = vertexSet.begin();
         i != vertexSet.end(); ++i) {
...
std::vector<std::map<int, std::string>> MetisPartitioner::partitioneWithGPMetis(
  ...
  if (partitionCount != "") {
    ...
    partitioner_logger.log(
        "Using the default partition count " + partitionCount, "info");
  }

  char buffer[128];
  std::string result = "";
  FILE *headerModify;
  std::string metisBinDir =
      utils.getJasmineGraphProperty("org.jasminegraph.partitioner.metis.bin");
  string metisCommand = metisBinDir + "/gpmetis " + this->outputFilePath +
                        "/grf " + to_string(this->nParts) + " 2>&1";
  ...
  if (input) {
    ...
    if (!result.empty() && result.find("Premature") != std::string::npos) {
      vertexCount -= 1;
      string newHeader =
          std::to_string(vertexCount) + ' ' + std::to_string(edgeCountForMetis);
      // string command = "sed -i \"1s/.*/" + newHeader +"/\" /tmp/grf";
      string command = "sed -i \"1s/.*/" + newHeader + "/\" " +
                       this->outputFilePath + "/grf";
      ...
      partitioneWithGPMetis(to_string(nParts));
               ...
               result.find("out of bounds") != std::string::npos) {
      vertexCount += 1;
      string newHeader =
          std::to_string(vertexCount) + ' ' + std::to_string(edgeCountForMetis);
      // string command = "sed -i \"1s/.*/" + newHeader +"/\" /tmp/grf";
      ...
      // However, I only found
               ...
               result.find("However, I only found") != std::string::npos) {
      ...
      string newEdgeSize =
          result.substr(first + firstDelimiter.length() + 1,
      ...
      partitioneWithGPMetis(to_string(nParts));
               ...
               result.find("Timing Information") != std::string::npos) {
      std::string line;
      string fileName =
          this->outputFilePath + "/grf.part." + to_string(this->nParts);
      ...
      string sqlStatement =
          "UPDATE graph SET vertexcount = '" +
          std::to_string(this->vertexCount) + "' ,centralpartitioncount = '" +
          std::to_string(this->nParts) + "' ,edgecount = '" +
...
void MetisPartitioner::createPartitionFiles(std::map<int, int> partMap) {
  ...
  partitioner_logger.log("Populating edge lists before writing to files",
                         "info");
  ...
  // Populate the masterEdgeLists with the remaining edges after thread
  // functions
  for (int part = 0; part < nParts; part++) {
    std::map<int, std::map<int, std::vector<int>>>
        commonCentralStoreEdgePartMap = commonCentralStoreEdgeMap[part];
    ...
    for (int subPart = 0; subPart < nParts; subPart++) {
      if (part == subPart) {
        ...
        std::map<int, std::vector<int>> partMasterEdgesSet =
            duplicateMasterGraphStorageMap[subPart];
        std::map<int, std::vector<int>> commonMasterEdgeSet =
            commonCentralStoreEdgePartMap[subPart];
        for (edgeMapIterator = commonMasterEdgeSet.begin();
          ...
          std::vector<int> centralGraphVertexVector =
              partMasterEdgesSet[edgeMapIterator->first];
          ...
          for (secondVertexIterator = secondVertexVector.begin();
               secondVertexIterator != secondVertexVector.end();
               ++secondVertexIterator) {
  ...
  if (graphAttributeType == Conts::GRAPH_WITH_TEXT_ATTRIBUTES ||
      graphType == Conts::GRAPH_TYPE_RDF) {
    threadCount = nParts * 5;
  }
  std::thread *threads = new std::thread[threadCount];
  count = 0;
  for (int part = 0; part < nParts; part++) {
    // threads[count] = std::thread(&MetisPartitioner::writePartitionFiles,
    // this, part);
    threads[count] = std::thread(
        &MetisPartitioner::writeSerializedPartitionFiles, this, part);
    count++;
    // threads[count] = std::thread(&MetisPartitioner::writeMasterFiles, this,
    // part);
    threads[count] =
        std::thread(&MetisPartitioner::writeSerializedMasterFiles, this, part);
    count++;
    threads[count] = std::thread(
        &MetisPartitioner::writeSerializedDuplicateMasterFiles, this, part);
    count++;
    if (graphAttributeType == Conts::GRAPH_WITH_TEXT_ATTRIBUTES) {
      threads[count] = std::thread(
          &MetisPartitioner::writeTextAttributeFilesForPartitions, this, part);
      count++;
      threads[count] = std::thread(
          &MetisPartitioner::writeTextAttributeFilesForMasterParts, this, part);
      count++;
    }
    if (graphType == Conts::GRAPH_TYPE_RDF) {
      threads[count] = std::thread(
          &MetisPartitioner::writeRDFAttributeFilesForPartitions, this, part);
      count++;
      threads[count] = std::thread(
          &MetisPartitioner::writeRDFAttributeFilesForMasterParts, this, part);
      count++;
    }
  }
  ...
  for (int part = 0; part < nParts; part++) {
    ...
    // Below two lines of code will be used in creating the composite central
    // stores
    ...
    string sqlStatement =
        "UPDATE partition SET central_edgecount = '" +
        std::to_string(masterEdgeCount) + "', central_edgecount_with_dups = '" +
        std::to_string(masterEdgeCountWithDups) + "' WHERE graph_idgraph = '" +
        std::to_string(this->graphID) + "' AND idpartition = '" +
        std::to_string(part) + "'";
    dbLock.lock();
    this->sqlite.runUpdate(sqlStatement);
    dbLock.unlock();
  ...
  // Below code segment will create the composite central stores which uses for
  // aggregation

  if (nParts > Conts::COMPOSITE_CENTRAL_STORE_WORKER_THRESHOLD) {
    bool haveSwapped = true;

    for (unsigned j = 1; haveSwapped && j < centralStoreSizeVector.size();
         ++j) {
      haveSwapped = false;
    ...
    for (centralStoreSizeVectorIterator = centralStoreSizeVector.begin();
         centralStoreSizeVectorIterator != centralStoreSizeVector.end();
         ++centralStoreSizeVectorIterator) {
      ...
      for (int loop = 0; loop < Conts::NUMBER_OF_COMPOSITE_CENTRAL_STORES;
           loop++) {
        ...
        for (currentGroupIterator = currentGroup.begin();
             currentGroupIterator != currentGroup.end();
    ...
    for (partitionGroupsIterator = partitionGroups.begin();
         partitionGroupsIterator != partitionGroups.end();
      ...
      if (partitionList.size() > 0) {
        ...
        for (partitionListIterator = partitionList.begin();
             partitionListIterator != partitionList.end();
             ++partitionListIterator) {
          int partitionId = *partitionListIterator;
          aggregatePartitionId =
              std::to_string(partitionId) + "_" + aggregatePartitionId;

          std::map<int, std::vector<int>> currentStorageMap =
              masterGraphStorageMap[partitionId];
          std::map<int, std::vector<int>>::iterator currentStorageMapIterator;

          for (currentStorageMapIterator = currentStorageMap.begin();
               currentStorageMapIterator != currentStorageMap.end();
               ++currentStorageMapIterator) {
            int startVetex = currentStorageMapIterator->first;
            std::vector<int> secondVertexVector =
                currentStorageMapIterator->second;

            std::vector<int> compositeMapSecondVertexVector =
                tempCompositeMap[startVetex];
            std::vector<int>::iterator secondVertexVectorIterator;

            for (secondVertexVectorIterator = secondVertexVector.begin();
                 secondVertexVectorIterator != secondVertexVector.end();
                 ++secondVertexVectorIterator) {
        ...
        compositeMasterGraphStorageMap[adjustedAggregatePartitionId] =
            tempCompositeMap;
      }
    }
    ...
    for (compositeGraphIdListIterator = compositeGraphIdList.begin();
         compositeGraphIdListIterator != compositeGraphIdList.end();
         ++compositeGraphIdListIterator) {
      std::string compositeGraphId = *compositeGraphIdListIterator;

      compositeCopyThreads[compositeCopyCount] =
          std::thread(&MetisPartitioner::writeSerializedCompositeMasterFiles,
                      this, compositeGraphId);
      compositeCopyCount++;
    }
...
void MetisPartitioner::populatePartMaps(std::map<int, int> partMap, int part) {
  ...
  if (graphType == Conts::GRAPH_TYPE_NORMAL_REFORMATTED) {
    for (edgeMapIterator = graphEdgeMap.begin();
         edgeMapIterator != graphEdgeMap.end(); ++edgeMapIterator) {
      ...
      if (startVertexPart == part) {
        ...
        for (secondVertexIterator = secondVertexVector.begin();
          ...
          if (endVertexPart == part) {
            ...
            commonMasterEdgeSet[endVertexPart][startVertexActual].push_back(
                endVertexActual);
          }
        }
      ...
      if (startVertexPart == part) {
        ...
        for (secondVertexIterator = secondVertexVector.begin();
          ...
          if (endVertexPart == part) {
             ...
             * This adds the edge to the masterGraphStorageMap with key being
             * the part of vertex 1
             */
            centralGraphVertexVector.push_back(endVertex);

            /* We need to insert these central store edges to the
             * masterGraphStorageMap where the key is the second vertex's part.
             * But it cannot be done inside the thread as it generates a race
             * condition due to multiple threads trying to write to
             * masterGraphStorageMap's maps apart from the one assigned to the
             * thread. Therefore, we take all such edges to a separate data
             * structure and add them to the masterGraphStorageMap later by a
             * single thread (main thread)
             */
            commonMasterEdgeSet[endVertexPart][startVertex].push_back(
                endVertex);
          }
        }
  ...
  string sqlStatement =
      ...
      "(idpartition,graph_idgraph,vertexcount,central_vertexcount,edgecount) "
      "VALUES(\"" +
      std::to_string(part) + "\", \"" + std::to_string(this->graphID) +
      "\", \"" + std::to_string(partVertexCounts[part]) + "\",\"" +
      std::to_string(centralPartVertices.size()) + "\",\"" +
...
void MetisPartitioner::writeSerializedPartitionFiles(int part) {
  string outputFilePart = outputFilePath + "/" + std::to_string(this->graphID) +
                          "_" + std::to_string(part);

  std::map<int, std::vector<int>> partEdgeMap =
      partitionedLocalGraphStorageMap[part];
  ...
  partitioner_logger.log("Serializing done for local part " + to_string(part),
                         "info");
}

void MetisPartitioner::writeSerializedMasterFiles(int part) {
  string outputFilePartMaster = outputFilePath + "/" +
                                std::to_string(this->graphID) +
                                "_centralstore_" + std::to_string(part);

  std::map<int, std::vector<int>> partMasterEdgeMap =
      masterGraphStorageMap[part];

  JasmineGraphHashMapCentralStore *hashMapCentralStore =
      new JasmineGraphHashMapCentralStore();
  hashMapCentralStore->storePartEdgeMap(partMasterEdgeMap,
                                        outputFilePartMaster);
  ...
  partitioner_logger.log("Serializing done for central part " + to_string(part),
                         "info");
}
...
void MetisPartitioner::writeSerializedDuplicateMasterFiles(int part) {
  string outputFilePartMaster = outputFilePath + "/" +
                                std::to_string(this->graphID) +
                                "_centralstore_dp_" + std::to_string(part);

  std::map<int, std::vector<int>> partMasterEdgeMap =
      duplicateMasterGraphStorageMap[part];
  ...
  this->utils.compressFile(outputFilePartMaster);
  masterFileMutex.lock();
  centralStoreDuplicateFileList.insert(
      make_pair(part, outputFilePartMaster + ".gz"));
  masterFileMutex.unlock();
  partitioner_logger.log(
      "Serializing done for duplicate central part " + to_string(part), "info");
}
...
void MetisPartitioner::writeMasterFiles(int part) {
  ...
  if (!partMasterEdgeMap.empty()) {
    ...
    if (masterFile.is_open()) {
      for (auto it = partMasterEdgeMap.begin(); it != partMasterEdgeMap.end();
           ++it) {
        int vertex = it->first;
        std::vector<int> destinationSet = it->second;
...
void MetisPartitioner::writeTextAttributeFilesForPartitions(int part) {
  string attributeFilePart = outputFilePath + "/" +
                             std::to_string(this->graphID) + "_attributes_" +
                             std::to_string(part);
  ...
  partitioner_logger.log(
      "Attribute writing done for local part " + to_string(part), "info");
}

void MetisPartitioner::writeTextAttributeFilesForMasterParts(int part) {
  string attributeFilePartMaster =
      outputFilePath + "/" + std::to_string(this->graphID) +
      "_centralstore_attributes_" + std::to_string(part);
  ...
  centralStoreAttributeFileList.insert(
      make_pair(part, attributeFilePartMaster + ".gz"));
  masterAttrFileMutex.unlock();
  partitioner_logger.log(
      "Attribute writing done for central part " + to_string(part), "info");
}
...
void MetisPartitioner::writeRDFAttributeFilesForPartitions(int part) {
  ...
  JasmineGraphHashMapLocalStore *hashMapLocalStore =
      new JasmineGraphHashMapLocalStore();
  hashMapLocalStore->storeAttributes(partitionedEdgeAttributes,
                                     attributeFilePart);
...
void MetisPartitioner::writeRDFAttributeFilesForMasterParts(int part) {
  ...
  JasmineGraphHashMapLocalStore *hashMapLocalStore =
      new JasmineGraphHashMapLocalStore();
  hashMapLocalStore->storeAttributes(centralStoreEdgeAttributes,
                                     attributeFilePartMaster);
...
string MetisPartitioner::reformatDataSet(string inputFilePath, int graphID) {
  ...
  string outputFile = utils.getHomeDir() + "/.jasminegraph/tmp/" +
                      std::to_string(this->graphID) + "/" +
                      std::to_string(this->graphID);
...
void MetisPartitioner::loadContentData(string inputAttributeFilePath,
                                       string graphAttributeType, int graphID,
                                       string attrType = "") {
  ...
  if (graphAttributeType == Conts::GRAPH_WITH_TEXT_ATTRIBUTES) {
    ...
    // Infer attribute data type if not explicitly given from first line in
    // attribute file
    if (attrType == "") {
      ...
      for (vector<string>::iterator i = strArr.begin(); i != strArr.end();
           i++) {
        string value = *i;
        if (value.find(".") !=
            std::string::npos) {  // Check if contains decimal point (float)
          attrType = "float";
          break;
        } else {
    ...
    while (!str_line.empty()) {
      ...
      string sqlStatement =
          "UPDATE graph SET feature_count = '" +
          std::to_string(arrayFeatures.size() - 1) + "', feature_type = '" +
          attrType + "' WHERE idgraph = '" + std::to_string(graphID) + "'";
...
void MetisPartitioner::writeSerializedCompositeMasterFiles(std::string part) {
  string outputFilePartMaster = outputFilePath + "/" +
                                std::to_string(this->graphID) +
                                "_compositecentralstore_" + part;

  std::map<int, std::vector<int>> partMasterEdgeMap =
      compositeMasterGraphStorageMap[part];
  ...
  this->utils.compressFile(outputFilePartMaster);
  masterFileMutex.lock();
  for (graphIdIterator = graphIds.begin(); graphIdIterator != graphIds.end();
       ++graphIdIterator) {
    std::string graphId = *graphIdIterator;
    compositeCentralStoreFileList.insert(
        make_pair(std::atoi(graphId.c_str()), outputFilePartMaster + ".gz"));
  }
  masterFileMutex.unlock();
