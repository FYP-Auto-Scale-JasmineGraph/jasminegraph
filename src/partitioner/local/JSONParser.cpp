...
void JSONParser::jsonParse(string filePath) {
  ...
  this->outputFilePath =
      utils.getHomeDir() + "/.jasminegraph/tmp/JSONParser/output";
  const clock_t begin = clock();
  readFile();
  float time = float(clock() - begin) / CLOCKS_PER_SEC;
  jsonparser_logger.log("FieldMap size : " + to_string(fieldsMap.size()),
                        "info");
  ...
  while (std::getline(infile, line)) {
    ...
    if (!reader.parse(line, root)) {
      ...
      if (idFound != vertexToIDMap.end()) {
        ...
        attrFile << mapped_id << "\t";
        for (auto it = tempVectorOfZeros.begin(); it != tempVectorOfZeros.end();
             ++it) {
          attrFile << *it << "\t ";
        }
        attrFile << endl;
...
void JSONParser::readFile() {
  ...
  this->utils.createDirectory(utils.getHomeDir() +
                              "/.jasminegraph/tmp/JSONParser");
  this->utils.createDirectory(utils.getHomeDir() +
                              "/.jasminegraph/tmp/JSONParser/output");
