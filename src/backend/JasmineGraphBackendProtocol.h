...
extern const string
    HANDSHAKE;  // To notify the server the host name of this worker;
extern const string
    HANDSHAKE_OK;          // Response to say it is ready for handshaking.
...
extern const string SEND;  // Ask the client to send some data. This is used
                           // during a comminication session with the client.
extern const string ERROR;
extern const string OUT_DEGREE_DISTRIBUTION_FOR_PARTITION;
extern const string
    IN_DEGREE_DISTRIBUTION_FOR_PARTITION;  // This commad gets the in degree
                                           // distribution from the external
                                           // world to this partition
extern const string WORLD_ONLY_AUTHFLOW_FOR_PARTITION;
extern const string LOCAL_TO_WORLD_AUTHFLOW_FOR_PARTITION;
extern const string
    WORLD_TO_LOCAL_FLOW_FROMIDS;  // This command gets all the vertices
                                  // connected with the external graph and their
                                  // corresponding fromIDs
extern const string DONE;
extern const string PARTITIONS_ON_HOST;  // The command to get the list of
                                         // partitions on particular host
extern const string ACKNOWLEGE_MASTER;  // Command to acknowledge the master
                                        // stating that the worker has started
