...
void *backendservicesesion(void *dummyPt) {
  ...
  while (!loop) {
    ...
    if (line.compare(EXIT_BACKEND) == 0) {
      ...
      int result_wr =
          write(connFd, WORKER_INFO_SEND.c_str(), WORKER_INFO_SEND.size());
      if (result_wr < 0) {
      ...
      worker_info.erase(
          std::remove(worker_info.begin(), worker_info.end(), '\n'),
          worker_info.end());
      worker_info.erase(
          std::remove(worker_info.begin(), worker_info.end(), '\r'),
          worker_info.end());

      std::vector<std::string> strArr = Utils::split(worker_info, '|');

      std::string updateQuery =
          "update worker set status='started' where ip='" + strArr[0] +
          "' and server_port='" + strArr[1] + "';";
