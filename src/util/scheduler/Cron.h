...
inline void verify_and_set(const std::string &token,
                           const std::string &expression, int &field,
  ...
  if (token == "*")
    ...
    try {
      ...
      throw BadCronExpression("malformed cron string (`" + token +
                              "` not convertable to int): " + expression);
    }
    if (field < lower_bound || field > upper_bound) {
      std::ostringstream oss;
      oss << "malformed cron string ('" << token
          << "' must be <= " << upper_bound << " and >= " << lower_bound
          << "): " << expression;
...
class Cron {
 public:
  explicit Cron(const std::string &expression) {
    ...
    if (tokens.size() != 5)
      throw BadCronExpression("malformed cron string (must be 5 fields): " +
                              expression);
  ...
  Clock::time_point cron_to_next(
      const Clock::time_point from = Clock::now()) const {
    ...
    while (true) {
      if (month != -1 && next.tm_mon != month) {
        // add a month
        // if this will bring us over a year, increment the year instead and
        // reset the month
        if (next.tm_mon + 1 > 11) {
          next.tm_mon = 0;
          next.tm_year++;
