...
std::vector<int> PlacesToNodeMapper::getInstancePortsList(long placeId) {
  ...
  if (utils.is_number(nWorkers)) {
    ...
    node_logger.log(
        "Number of Workers specified in the properties is not an integer "
        "value.",
        "error");
    return portList;
  }
  ...
  if (placeId > 0 && (placeId + 1) <= hostListModeNWorkers) {
    workerPort =
        workerPort + (placeId * numberOfWorkersPerHost * 2) + placeId * 2 + 2;
  } else if (placeId > 0 && (placeId + 1) > hostListModeNWorkers) {
    workerPort = workerPort + (placeId * numberOfWorkersPerHost * 2) +
                 hostListModeNWorkers * 2 + 2;
  }
...
std::vector<int> PlacesToNodeMapper::getFileTransferServicePort(long placeId) {
  ...
  if (placeId > 0 && (placeId + 1) <= hostListModeNWorkers) {
    workerDataPort = workerDataPort + (placeId * numberOfWorkersPerHost * 2) +
                     placeId * 2 + 2;
  } else if (placeId > 0 && (placeId + 1) > hostListModeNWorkers) {
    workerDataPort = workerDataPort + (placeId * numberOfWorkersPerHost * 2) +
                     hostListModeNWorkers * 2 + 2;
