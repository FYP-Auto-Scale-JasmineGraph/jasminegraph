...
map<string, std::map<int, int>>
JasmineGraphTrainingSchedular::schedulePartitionTraining(std::string graphID) {
  ...
  string sqlStatement =
      "SELECT host_idhost, name FROM worker_has_partition INNER JOIN worker ON "
      "worker_idworker = "
      "idworker WHERE partition_graph_idgraph = " +
      graphID + " group by host_idhost";
  std::vector<vector<pair<string, string>>> result =
      refToSqlite.runSelect(sqlStatement);
  for (vector<vector<pair<string, string>>>::iterator i = result.begin();
       i != result.end(); ++i) {
    int count = 0;
    string hostID;
    string hostname;
  ...
  trainScheduler_logger.log("Scheduling training order for each worker",
                            "info");
  for (std::vector<pair<string, string>>::iterator j = (hostData.begin());
       j != hostData.end(); ++j) {
    sqlStatement =
        "SELECT idpartition, vertexcount, central_vertexcount, graph_idgraph "
        "FROM partition INNER JOIN "
        "(SELECT host_idhost, partition_idpartition, partition_graph_idgraph, "
        "worker_idworker FROM "
        "worker_has_partition INNER JOIN worker ON worker_idworker = idworker) "
        "AS a ON partition.idpartition = "
    ...
    for (std::vector<vector<pair<string, string>>>::iterator i =
      ...
      vertexCountToPartitionId.insert(
          {vertexcount + centralVertexCount, partition_id});
    }
    long availableMemory = getAvailableMemory(j->second);
    vector<pair<int, int>> memoryEstimationForEachPartition;
    for (auto i = vertexCountToPartitionId.begin();
         i != vertexCountToPartitionId.end(); ++i) {
      long memoryForPartition = estimateMemory(i->first, graphID);
      trainScheduler_logger.log(
          "Estimated memory for partition :" + to_string(i->second) + " is " +
              to_string(memoryForPartition),
          "info");
      if (memoryForPartition > availableMemory) {
        memoryForPartition = availableMemory;
      }
      memoryEstimationForEachPartition.push_back(
          make_pair(i->second, (int)memoryForPartition));
    }
    std::map<int, int> scheduledPartitionSets = packPartitionsToMemory(
        memoryEstimationForEachPartition, availableMemory);
    scheduleForEachHost.insert(make_pair(j->second, scheduledPartitionSets));
  }
  return scheduleForEachHost;
}

long JasmineGraphTrainingSchedular::estimateMemory(int vertexCount,
                                                   string graph_id) {
  SQLiteDBInterface refToSqlite = *new SQLiteDBInterface();
  refToSqlite.init();
  ...
  for (std::vector<vector<pair<string, string>>>::iterator i = result.begin();
       i != result.end(); ++i) {
    std::vector<pair<string, string>> rowData = *i;
  ...
  long totalMemoryApproximation =
      (featureMatrixSize + adjacencyMatrixSize + degreeMatrixSize +
       embeddingMatrixSize + networkXgraphsize) /
      1024;
...
long JasmineGraphTrainingSchedular::getAvailableMemory(string hostname) {
  PerformanceSQLiteDBInterface refToPerfDb =
      *new PerformanceSQLiteDBInterface();
  refToPerfDb.init();
  trainScheduler_logger.log("Fetching available host " + hostname + " memory",
                            "info");
  string perfSqlStatement =
      "SELECT memory_usage FROM host_performance_data INNER JOIN (SELECT "
      "idhost FROM host WHERE ip = '" +
      hostname + "') USING (idhost) ORDER BY date_time DESC LIMIT 1";
  vector<vector<pair<string, string>>> result =
      refToPerfDb.runSelect(perfSqlStatement);
  if (result.size() == 0) {
    return 0;
  }
...
std::map<int, int> JasmineGraphTrainingSchedular::packPartitionsToMemory(
    vector<pair<int, int>> partitionMemoryList, int capacity) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < res; j++) {
      if (bin_rem[j] >= partitionMemoryList[i].second &&
          bin_rem[j] - partitionMemoryList[i].second < min) {
        bestBin = j;
  ...
  trainScheduler_logger.log("Packing partitions to fit memory completed",
                            "info");
 ...
 *  @return Map of host to maps containing schedule for that host given by
 * schedulePartitionsBestFit method
 */
map<string, std::map<int, map<int, int>>>
JasmineGraphTrainingSchedular::scheduleGradientPassingTraining(
    std::string graphID) {
  ...
  string sql =
      "SELECT feature_count, feature_type FROM graph WHERE idgraph = " +
      graphID;
  std::vector<vector<pair<string, string>>> graphResult =
      refToSqlite.runSelect(sql);
  ...
  for (std::vector<pair<string, string>>::iterator j = (hostData.begin());
    ...
    sqlStatement =
        ...
        "central_edgecount, worker_idworker FROM partition INNER JOIN "
        "(SELECT host_idhost, partition_idpartition, partition_graph_idgraph, "
        "worker_idworker FROM "
        "worker_has_partition INNER JOIN worker ON worker_idworker = idworker) "
        "AS a ON partition.idpartition = "
    ...
    vector<vector<int>>
        partitionMetadata;  // Vector for node count, edge count, feature count
                            // for memory estimation of each partition
    map<int, int> partitionWorkerMap;
    // Iterate through partition list and get partition details
    trainScheduler_logger.log("Commence schedule creation for host" + j->second,
                              "info");
    for (std::vector<vector<pair<string, string>>>::iterator i =
             results.begin();
         i != results.end(); ++i) {
      ...
      vector<int> partitionValues;  // Vector for partition node count, edge
    ...
    long availableMemory = getAvailableMemory(j->second);  // Host memory (in
                                                           // KB)

    // Get memory estimation list for each partition of host
    vector<pair<int, double>> partitionMemoryList =
        estimateMemoryDistOpt(partitionMetadata, availableMemory);

    // Get schedule for host
    map<int, map<int, int>> scheduledPartitionSets = schedulePartitionsBestFit(
        partitionMemoryList, partitionWorkerMap, availableMemory);
    scheduleForEachHost.insert(make_pair(j->second, scheduledPartitionSets));
  }
  return scheduleForEachHost;
}
 ...
 *
 * @param partitionMetadata a vector of vectors containing partition ID, node
 * count, edge count, feature count, feature size (in bytes) per partition
 * @param availableMemory total memory of host (in KB)
 * @return Vector of pairs containing partition ids and estimated memory (in KB)
 * for distributed opt training process
 */
vector<pair<int, double>> JasmineGraphTrainingSchedular::estimateMemoryDistOpt(
    vector<vector<int>> partitionMetadata, long availableMemory) {
  vector<pair<int, double>>
      partitionMemoryList;  // Vector of estimated sizes of partitions

  trainScheduler_logger.log("Estimating host partition size in memory", "info");
  // Iterate through partitions and estimate sizes
  for (vector<vector<int>>::iterator i = partitionMetadata.begin();
       i != partitionMetadata.end(); i++) {
    int partitionID = (*i)[0];
    int nodeCount = (*i)[1];
    int edgeCount = (*i)[2];
    ...
    double graphSize = ((double)featureSize) * nodeCount * edgeCount *
                       featureCount / (1024 * 1024);  // Size in hard disk (KB)

    // The following linear relationship was observed between the computed graph
    // size and graph size in memory in the experiments
    double sizeInMem =
        3.6 * graphSize + 2 * 1024;  // Size when loaded into memory
 ...
 *
 * @param partitionMemoryList Vector of pairs containing partition ID and
 * estimated size in memory
 * @param partitionWorkerMap Map of partition id and the worker assigned to
 * partition
 * @param capacity total memory of host
 * @return Map from worker to maps from partition to order of loading into
 * memory
 */
map<int, map<int, int>>
JasmineGraphTrainingSchedular::schedulePartitionsBestFit(
    vector<pair<int, double>> partitionMemoryList,
    map<int, int> partitionWorkerMap, int capacity) {
  std::map<int, map<int, int>> schedule;  // Host schedule per worker and what
                                          // partitions to load in which order
  ...
  cout << "No of graph partitions in host " << partitionMemoryList.size()
       << endl;

  int order = 0;  // Order of partitions being loaded into memory
  double remainingMem = (double)capacity;  // Total remaining mem after each
                                           // pass (iteration) in simulation
  vector<pair<int, double>>
      simulatedMem;  // Partitions in mem and their remaining work (in memory)
                     // in simulation
  ...
  while (schedule.size() < partitionMemoryList.size()) {
    ...
    for (vector<pair<int, double>>::iterator i = partitionMemoryList.begin();
         i != partitionMemoryList.end(); i++) {
    ...
    double minRemainingWork =
        min_element(simulatedMem.begin(), simulatedMem.end(),
    ...
    for (vector<pair<int, double>>::iterator i = simulatedMem.begin();
         i != simulatedMem.end(); i++) {
      int partition = i->first;
      double remainingWork = i->second;
      int worker = partitionWorkerMap[partition];
      if (remainingWork <= minRemainingWork) {
        // Free memory taken by partition
        for (vector<pair<int, double>>::iterator j =
                 partitionMemoryList.begin();
        ...
        // Remove partition entry from simulated memory and update iterator
        // pointer
