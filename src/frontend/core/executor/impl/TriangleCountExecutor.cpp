...
std::map<long, std::map<long, std::vector<long>>>
    TriangleCountExecutor::triangleTree;
...
void TriangleCountExecutor::execute() {
  ...
  std::string graphSLAString =
      request.getParameter(Conts::PARAM_KEYS::GRAPH_SLA);
  ...
  triangleCount_logger.log(
      "###TRIANGLE-COUNT-EXECUTOR### Started with graph ID : " + graphId +
          " Master IP : " + masterIP,
      "info");
  ...
  string sqlStatement =
      ...
      "name,ip,user,server_port,server_data_port,partition_idpartition "
  ...
  if (isCompositeAggregation) {
    ...
    std::vector<std::string> graphFiles =
        utils.getListOfFilesInDirectory(aggregatorFilePath);

    std::vector<std::string>::iterator graphFilesIterator;
    std::string compositeFileNameFormat = graphId + "_compositecentralstore_";

    for (graphFilesIterator = graphFiles.begin();
         graphFilesIterator != graphFiles.end(); ++graphFilesIterator) {
  ...
  for (std::vector<vector<pair<string, string>>>::iterator i = results.begin();
       i != results.end(); ++i) {
    ...
    if (partitionMap.find(workerID) == partitionMap.end()) {
      ...
      partitionMap.insert(
          std::pair<string, std::vector<string>>(workerID, partitionVec));
    } else {
    ...
    triangleCount_logger.log(
        "###TRIANGLE-COUNT-EXECUTOR### Getting Triangle Count : Host " + host +
  ...
  for (int i = 0; i < workerListSize; i++) {
    ...
    for (partitionIterator = partitionList.begin();
         partitionIterator != partitionList.end(); ++partitionIterator) {
      partitionCount++;
      ...
      intermRes.push_back(std::async(
          std::launch::async, TriangleCountExecutor::getTriangleCount,
  ...
  std::string query =
      "SELECT attempt from graph_sla INNER JOIN sla_category where "
      "graph_sla.id_sla_category=sla_category.id and "
      "graph_sla.graph_id='" +
      graphId + "' and graph_sla.partition_count='" +
      std::to_string(partitionCount) + "' and sla_category.category='" +
      Conts::SLA_CATEGORY::LATENCY + "';";

  std::vector<vector<pair<string, string>>> queryResults =
      perfDB.runSelect(query);
  ...
  if (queryResults.size() > 0) {
    ...
    triangleCount_logger.log(
        "###TRIANGLE-COUNT-EXECUTOR### Inserting initial record for SLA ",
        "info");
    Utils::updateSLAInformation(perfDB, graphId, partitionCount, 0, TRIANGLES,
                                Conts::SLA_CATEGORY::LATENCY);
    statResponse.push_back(std::async(
        std::launch::async, TriangleCountExecutor::collectPerformaceData,
        perfDB, graphId.c_str(), TRIANGLES, Conts::SLA_CATEGORY::LATENCY,
        partitionCount, masterIP));
    isStatCollect = true;
  }
  ...
  if (!isCompositeAggregation) {
    long aggregatedTriangleCount =
        TriangleCountExecutor::aggregateCentralStoreTriangles(
            sqlite, graphId, masterIP, threadPriority);
    result += aggregatedTriangleCount;
    workerResponded = true;
    triangleCount_logger.log(
        "###TRIANGLE-COUNT-EXECUTOR### Getting Triangle Count : Completed: "
        "Triangles " +
            to_string(result),
        "info");
  }
  ...
  jobResponse.addParameter(Conts::PARAM_KEYS::TRIANGLE_COUNT,
  ...
  if (canCalibrate) {
    Utils::updateSLAInformation(perfDB, graphId, partitionCount, msDuration,
  ...
  for (processCompleteIterator = processData.begin();
...
std::vector<std::vector<string>> TriangleCountExecutor::getCombinations(
    std::vector<string> inputVector) {
  std::vector<std::vector<string>> combinationsList;
  std::vector<std::vector<int>> combinations;

  // Below algorithm will get all the combinations of 3 workers for given set of
  // workers
  ...
  for (std::vector<std::vector<int>>::iterator combinationsIterator =
           combinations.begin();
...
long TriangleCountExecutor::getTriangleCount(int graphId, std::string host,
                                             int port, int dataPort,
                                             int partitionId,
                                             std::string masterIP, int uniqueId,
  ...
  triangleCount_logger.log(
      "###TRIANGLE-COUNT-EXECUTOR### Get Host By Name : " + host, "info");
  ...
  triangleCount_logger.log("Sent : " + JasmineGraphInstanceProtocol::HANDSHAKE,
                           "info");
  bzero(data, 301);
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    triangleCount_logger.log(
        "Sent : " + JasmineGraphInstanceProtocol::TRIANGLES, "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      triangleCount_logger.log("Sent : Graph ID " + std::to_string(graphId),
                               "info");
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      triangleCount_logger.log(
          "Sent : Thread Priority " + std::to_string(threadPriority), "info");
    ...
    if (isCompositeAggregation) {
      triangleCount_logger.log("###COMPOSITE### Started Composite aggregation ",
                               "info");
      static std::vector<std::vector<string>>::iterator combinationsIterator;

      for (int combinationIndex = 0; combinationIndex < fileCombinations.size();
           ++combinationIndex) {
        ...
        for (listIterator = fileList.begin(); listIterator != fileList.end();
             ++listIterator) {
          ...
          std::vector<std::string> fileNameParts =
              utils.split(rawFileName, '_');

          /*Partition numbers are extracted from  the file name. The starting
           * index of partition number is 2. Therefore the loop starts with 2*/
          for (int index = 2; index < fileNameParts.size(); ++index) {
            partitionSet.insert(fileNameParts[index]);
          }
        }

        if (partitionSet.find(std::to_string(partitionId)) ==
            partitionSet.end()) {
          continue;
        }
        ...
        for (fileListIterator = fileList.begin();
             fileListIterator != fileList.end(); ++fileListIterator) {
        ...
        std::string adjustedCombinationKey =
            combinationKey.substr(0, combinationKey.size() - 1);
        std::string adjustedAvailableFiles =
            availableFiles.substr(0, availableFiles.size() - 1);
        std::string adjustedTransferredFile =
            transferredFiles.substr(0, transferredFiles.size() - 1);

        fileCombinationMutex.lock();
        if (combinationWorkerMap.find(combinationKey) ==
            combinationWorkerMap.end()) {
          if (partitionIdSet.find(std::to_string(partitionId)) !=
              partitionIdSet.end()) {
        ...
        if (isAggregateValid) {
          ...
          for (transferRequireFileIterator = transferRequireFiles.begin();
               transferRequireFileIterator != transferRequireFiles.end();
               ++transferRequireFileIterator) {
            std::string transferFileName = *transferRequireFileIterator;
            std::string fileAccessible = isFileAccessibleToWorker(
                std::to_string(graphId), std::string(), host,
                std::to_string(port), masterIP,
                JasmineGraphInstanceProtocol::FILE_TYPE_CENTRALSTORE_COMPOSITE,
                transferFileName);

            if (fileAccessible.compare("false") == 0) {
              copyCompositeCentralStoreToAggregator(host, std::to_string(port),
                                                    std::to_string(dataPort),
                                                    transferFileName, masterIP);
            }
          }

          std::string compositeTriangles = countCompositeCentralStoreTriangles(
              host, std::to_string(port), adjustedTransferredFile, masterIP,
              adjustedAvailableFiles, threadPriority);

          triangleCount_logger.log(
              "###COMPOSITE### Retrieved Composite triangle list ", "debug");

          std::vector<std::string> triangles =
              Utils::split(compositeTriangles, ':');
    ...
    triangleCount_logger.log(
        "###COMPOSITE### Returning Total Triangles from executer ", "debug");
...
bool TriangleCountExecutor::proceedOrNot(std::set<string> partitionSet,
                                         int partitionId) {
  const std::lock_guard<std::mutex> lock(aggregateWeightMutex);

  std::set<string>::iterator partitionSetIterator;
  std::map<int, int> tempWeightMap;
  for (partitionSetIterator = partitionSet.begin();
       partitionSetIterator != partitionSet.end(); ++partitionSetIterator) {
  ...
  pair<int, int> entryWithMinValue =
      make_pair(partitionId, currentWorkerWeight);

  map<int, int>::iterator currentEntry;

  for (currentEntry = aggregateWeightMap.begin();
       currentEntry != aggregateWeightMap.end(); ++currentEntry) {
  ...
  if (entryWithMinValue.first == partitionId) {
    ...
    triangleCount_logger.log(
        "###COMPOSITE### Aggregator Initiated : Partition ID: " +
...
void TriangleCountExecutor::updateMap(int partitionId) {
  ...
  triangleCount_logger.log(
      "###COMPOSITE### Aggregator Completed : Partition ID: " +
          std::to_string(partitionId) +
          " Weight : " + std::to_string(currentWeight),
      "info");
}

int TriangleCountExecutor::updateTriangleTreeAndGetTriangleCount(
    std::vector<std::string> triangles) {
  ...
  for (triangleIterator = triangles.begin();
       triangleIterator != triangles.end(); ++triangleIterator) {
    ...
    if (!triangle.empty() && triangle != "NILL") {
      ...
      std::map<long, std::vector<long>>::iterator itemResIterator =
          itemRes.find(vertexTwo);
...
long TriangleCountExecutor::aggregateCentralStoreTriangles(
    SQLiteDBInterface sqlite, std::string graphId, std::string masterIP,
    int threadPriority) {
  std::vector<std::vector<string>> workerCombinations =
      getWorkerCombination(sqlite, graphId);
  ...
  for (workerCombinationsIterator = workerCombinations.begin();
       workerCombinationsIterator != workerCombinations.end();
       ++workerCombinationsIterator) {
    ...
    for (workerCombinationIterator = workerCombination.begin();
         workerCombinationIterator != workerCombination.end();
         ++workerCombinationIterator) {
    ...
    std::vector<vector<pair<string, string>>> result =
        sqlite.runSelect(aggregatorSqlStatement);
    ...
    if ((aggregatorIp.find("localhost") != std::string::npos) ||
        aggregatorIp == masterIP) {
    ...
    for (aggregatorCopyCombinationIterator = workerCombination.begin();
         aggregatorCopyCombinationIterator != workerCombination.end();
         ++aggregatorCopyCombinationIterator) {
      ...
      if (workerId != minWeightWorker) {
        ...
        if ((workerIp.find("localhost") != std::string::npos) ||
            workerIp == masterIP) {
          host = workerIp;
        } else {
        ...
        if (centralStoreAvailable.compare("false") == 0) {
          remoteGraphCopyResponse.push_back(std::async(
              std::launch::async,
              TriangleCountExecutor::copyCentralStoreToAggregator,
              aggregatorHost, aggregatorPort, aggregatorDataPort,
    ...
    std::string adjustedPartitionIdList =
        partitionIdList.substr(0, partitionIdList.size() - 1);
    workerWeightMap[minWeightWorker] = minimumWeight;

    triangleCountResponse.push_back(std::async(
        std::launch::async, TriangleCountExecutor::countCentralStoreTriangles,
        aggregatorHost, aggregatorPort, aggregatorHost, aggregatorPartitionId,
        adjustedPartitionIdList, graphId, masterIP, threadPriority));
  }
...
string TriangleCountExecutor::isFileAccessibleToWorker(
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd, JasmineGraphInstanceProtocol::CHECK_FILE_ACCESSIBLE.c_str(),
...
std::string TriangleCountExecutor::copyCompositeCentralStoreToAggregator(
    std::string aggregatorHostName, std::string aggregatorPort,
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::SEND_COMPOSITE_CENTRALSTORE_TO_AGGREGATOR
            .c_str(),
        JasmineGraphInstanceProtocol::SEND_COMPOSITE_CENTRALSTORE_TO_AGGREGATOR
            .size());
    ...
    if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_NAME) == 0) {
      triangleCount_logger.log(
          "Received : " + JasmineGraphInstanceProtocol::SEND_FILE_NAME, "info");
      ...
      if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_LEN) == 0) {
        ...
        if (response.compare(JasmineGraphInstanceProtocol::SEND_FILE_CONT) ==
          ...
          triangleCount_logger.log(
              ...
              "info");
          triangleCount_logger.log("Going to send file through service",
                                   "info");
          jasmineServer->sendFileThroughService(
              aggregatorHostName, std::atoi(aggregatorDataPort.c_str()),
    ...
    while (true) {
      ...
      if (response.compare(JasmineGraphInstanceProtocol::FILE_RECV_WAIT) == 0) {
        triangleCount_logger.log(
            ...
            "info");
        triangleCount_logger.log("Checking file status : " + to_string(count),
                                 "info");
        count++;
        sleep(1);
        continue;
        ...
        triangleCount_logger.log(
            "File transfer completed for file : " + aggregateStoreFile, "info");
        break;
      }
    }
    ...
    while (true) {
      ...
      if (response.compare(JasmineGraphInstanceProtocol::BATCH_UPLOAD_WAIT) ==
        ...
        triangleCount_logger.log(
            ...
            "info");
        triangleCount_logger.log("CentralStore partition file upload completed",
                                 "info");
        break;
      }
    }
...
string TriangleCountExecutor::countCompositeCentralStoreTriangles(
    std::string aggregatorHostName, std::string aggregatorPort,
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::AGGREGATE_COMPOSITE_CENTRALSTORE_TRIANGLES
            .c_str(),
        JasmineGraphInstanceProtocol::AGGREGATE_COMPOSITE_CENTRALSTORE_TRIANGLES
            .size());
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      triangleCount_logger.log("Received : " + JasmineGraphInstanceProtocol::OK,
                               "info");
      result_wr =
          write(sockfd, availableFileList.c_str(), availableFileList.size());
      ...
      triangleCount_logger.log(
          "Sent : Available File List " + availableFileList, "info");
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      for (unsigned i = 0; i < compositeCentralStoreFileList.length();
           i += INSTANCE_DATA_LENGTH - 10) {
        std::string chunk =
            compositeCentralStoreFileList.substr(i, INSTANCE_DATA_LENGTH - 10);
        if (i + INSTANCE_DATA_LENGTH - 10 <
            compositeCentralStoreFileList.length()) {
          chunk += "/SEND";
        } else {
          chunk += "/CMPT";
        }
...
std::vector<std::vector<string>> TriangleCountExecutor::getWorkerCombination(
  ...
  std::vector<std::vector<string>> workerIdCombination =
      getCombinations(workerIdVector);

  return workerIdCombination;
}

std::string TriangleCountExecutor::copyCentralStoreToAggregator(
    std::string aggregatorHostName, std::string aggregatorPort,
  ...
  std::string aggregatorFilePath = utils.getJasmineGraphProperty(
      "org.jasminegraph.server.instance.aggregatefolder");
  std::string fileName = std::to_string(graphId) + "_centralstore_" +
                         std::to_string(partitionId) + ".gz";
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::SEND_CENTRALSTORE_TO_AGGREGATOR.c_str(),
    ...
    while (true) {
      ...
      if (response.compare(JasmineGraphInstanceProtocol::FILE_RECV_WAIT) == 0) {
        ...
        triangleCount_logger.log(
            "Received : " + JasmineGraphInstanceProtocol::FILE_ACK, "info");
        triangleCount_logger.log(
            "File transfer completed for file : " + centralStoreFile, "info");
        break;
      }
    }
...
string TriangleCountExecutor::countCentralStoreTriangles(
    std::string aggregatorHostName, std::string aggregatorPort,
  ...
  if (response.compare(JasmineGraphInstanceProtocol::HANDSHAKE_OK) == 0) {
    ...
    result_wr = write(
        sockfd,
        JasmineGraphInstanceProtocol::AGGREGATE_CENTRALSTORE_TRIANGLES.c_str(),
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      ...
      triangleCount_logger.log("Sent : Partition ID List : " + partitionId,
                               "info");
...
int TriangleCountExecutor::collectPerformaceData(
    PerformanceSQLiteDBInterface perDB, std::string graphId,
  ...
  std::string slaCategoryId =
      performanceUtil.getSLACategoryId(command, category);
  ...
  for (placeListIterator = placeList.begin();
    ...
    if (!(isMaster.find("true") != std::string::npos || host == "localhost" ||
          host.compare(masterIP) == 0)) {
      performanceUtil.initiateCollectingRemoteSLAResourceUtilization(
          host, atoi(serverPort.c_str()), isHostReporter, "false", placeId,
          elapsedTime, masterIP);
    }
  }
  ...
  while (!workerResponded) {
    if (time(0) - start == Conts::LOAD_AVG_COLLECTING_GAP) {
      ...
      performanceUtil.collectSLAResourceConsumption(placeList, graphId,
                                                    masterIP, elapsedTime);
      start = start + Conts::LOAD_AVG_COLLECTING_GAP;
    }
  }

  performanceUtil.updateRemoteResourceConsumption(
      perDB, graphId, partitionCount, placeList, slaCategoryId, masterIP);
  performanceUtil.updateResourceConsumption(perDB, graphId, partitionCount,
                                            placeList, slaCategoryId);
