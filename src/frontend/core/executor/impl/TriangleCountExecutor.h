...
class TriangleCountExecutor : public AbstractExecutor {
 public:
  ...
  TriangleCountExecutor(SQLiteDBInterface db,
  ...
  static std::vector<std::vector<string>> getCombinations(
      std::vector<string> inputVector);

  static long getTriangleCount(int graphId, std::string host, int port,
                               int dataPort, int partitionId,
                               std::string masterIP, int uniqueId,
                               bool isCompositeAggregation, int threadPriority);

  static long aggregateCentralStoreTriangles(SQLiteDBInterface sqlite,
                                             std::string graphId,
                                             std::string masterIP,
                                             int threadPriority);

  static string isFileAccessibleToWorker(
  ...
  static int collectPerformaceData(PerformanceSQLiteDBInterface perDB,
                                   std::string graphId, std::string command,
                                   std::string category, int partitionCount,
                                   std::string masterIP);
