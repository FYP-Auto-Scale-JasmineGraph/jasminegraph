...
JobScheduler::JobScheduler(SQLiteDBInterface sqlite,
...
void *startScheduler(void *dummyPt) {
  ...
  while (true) {
    if (jobQueue.size() > 0) {
      jobScheduler_Logger.log("##JOB SCHEDULER## Jobs Available for Scheduling",
                              "info");
      ...
      while (!jobQueue.empty()) {
        ...
        if (request.getPriority() == Conts::HIGH_PRIORITY_DEFAULT_VALUE &&
          ...
          jobQueue.pop();
          JobScheduler::processJob(request, refToScheduler->sqlite,
      ...
      if (pendingHPJobList.size() > 0) {
        jobScheduler_Logger.log(
            "##JOB SCHEDULER## High Priority Jobs in Queue: " +
        ...
        std::string category =
            pendingHPJobList[0].getParameter(Conts::PARAM_KEYS::CATEGORY);

        std::vector<long> scheduleTimeVector =
            performanceUtil.getResourceAvailableTime(
                highPriorityGraphList, jobType, category, masterIP,
                pendingHPJobList);
      ...
      std::this_thread::sleep_for(
          std::chrono::seconds(Conts::SCHEDULER_SLEEP_TIME));
    }
  }
  return NULL;
}
...
void JobScheduler::processJob(JobRequest request, SQLiteDBInterface sqlite,
                              PerformanceSQLiteDBInterface perfDB) {
  intermRes.push_back(std::async(std::launch::async, JobScheduler::executeJob,
                                 request, sqlite, perfDB));
}

void JobScheduler::executeJob(JobRequest request, SQLiteDBInterface sqlite,
                              PerformanceSQLiteDBInterface perfDB) {
