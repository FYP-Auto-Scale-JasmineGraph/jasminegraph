...
// Thread function
void listen_to_kafka_topic(KafkaConnector *kstream,
  ...
  while (true) {
    ...
    if (data == "-1") {  // Marks the end of stream
      frontend_logger.log(
          "Received the end of `" + stream_topic_name + "` input kafka stream",
          "info");
      break;
    }
    ...
    workerClients.at((int)partitionedEdge[0].second)
        ->publish(sourceJson.dump());
    workerClients.at((int)partitionedEdge[1].second)
...
void *frontendservicesesion(std::string masterIP, int connFd,
  ...
  while (!loop) {
    if (currentFESession == Conts::MAX_FE_SESSIONS + 1) {
      ...
      std::string errorResponse =
          "Jasminegraph Server is Busy. Please try again later.";
      int result_wr =
          write(connFd, errorResponse.c_str(), errorResponse.length());
      if (result_wr < 0) {
    ...
    if (line.compare(EXIT) == 0) {
      ...
      std::vector<vector<pair<string, string>>> v = sqlite.runSelect(
          "SELECT idgraph, name, upload_path, graph_status_idgraph_status FROM "
          "graph;");
        ...
        for (std::vector<pair<string, string>>::iterator j = (i->begin());
          ...
          if (counter == 3) {
            if (std::stoi(j->second) == Conts::GRAPH_STATUS::LOADING) {
              ...
              ss << "deleting|";
            } else if (std::stoi(j->second) ==
                       Conts::GRAPH_STATUS::NONOPERATIONAL) {
              ss << "nop|";
            } else if (std::stoi(j->second) ==
      ...
      if (utils.fileExists(path)) {
        ...
        string sqlStatement =
            "INSERT INTO graph "
            "(name,upload_path,upload_start_time,upload_end_time,graph_status_"
            "idgraph_status,"
        ...
        string input_file_path = utils.getHomeDir() + "/.jasminegraph/tmp/" +
        ...
        JasmineGraphServer *jasmineServer = new JasmineGraphServer();
        jasmineServer->uploadGraphLocally(
            newGraphID, Conts::GRAPH_WITH_ATTRIBUTES, fullFileList, masterIP);
        utils.deleteDirectory(utils.getHomeDir() + "/.jasminegraph/tmp/" +
                              to_string(newGraphID));
        utils.deleteDirectory("/tmp/" + std::to_string(newGraphID));
        JasmineGraphFrontEnd::getAndUpdateUploadTime(to_string(newGraphID),
                                                     sqlite);
      } else {
        frontend_logger.log(
            "Graph data file does not exist on the specified path", "error");
        continue;
      }
      ...
      if (utils.fileExists(path)) {
        ...
        if (result == 0) {
          string reformattedFilePath =
              partitioner->reformatDataSet(path, newGraphID);
          partitioner->loadDataSet(reformattedFilePath, newGraphID);
          partitioner->constructMetisFormat(
              Conts::GRAPH_TYPE_NORMAL_REFORMATTED);
      ...
      if (utils.fileExists(path)) {
        frontend_logger.log("Path exists", "info");
        std::string toDir =
            utils.getJasmineGraphProperty("org.jasminegraph.server.modelDir");
        utils.copyToDirectory(path, toDir);

        string sqlStatement =
            "INSERT INTO model "
            "(name,upload_path,upload_time,model_status_idmodel_status"
            ")VALUES(\"" +
            name + "\", \"" + path + "\", \"" + uploadStartTime + "\",\"" +
            to_string(Conts::GRAPH_STATUS::LOADING) + "\")";
        ...
        frontend_logger.log("Model file does not exist on the specified path",
                            "error");
        continue;
      }
      ...
      // We get the name and the path to graph edge list and attribute list as a
      // triplet separated by | . <name>|<path to edge list>|<path to attribute
      // file>|(optional)<attribute data type: int8. int16, int32 or float> Data
      // types based on numpy array data types for numerical values with int8
      // referring to 8bit integers etc. If data type is not specified, it will
      // be inferred from values present in the first line of the attribute file
      // The provided data type should be the largest in the following order:
      // float > int32 > int16 > int8 Inferred data type will be the largest
      // type based on the values present in the attribute file first line
      message =
          "Send <name>|<path to edge list>|<path to attribute "
          "file>|(optional)<attribute data type: int8. int16, int32 or "
          "float>\r\n";
      ...
      if (strArr.size() == 4) {
        ...
        if (attrDataType != "int8" && attrDataType != "int16" &&
            attrDataType != "int32" && attrDataType != "float") {
      ...
      if (utils.fileExists(edgeListPath) &&
          utils.fileExists(attributeListPath)) {
        ...
        partitioner->loadContentData(attributeListPath, graphAttributeType,
                                     newGraphID, attrDataType);
        partitioner->loadDataSet(edgeListPath, newGraphID);
        int result =
            partitioner->constructMetisFormat(Conts::GRAPH_TYPE_NORMAL);
        if (result == 0) {
          string reformattedFilePath =
              partitioner->reformatDataSet(edgeListPath, newGraphID);
          partitioner->loadDataSet(reformattedFilePath, newGraphID);
          partitioner->constructMetisFormat(
              Conts::GRAPH_TYPE_NORMAL_REFORMATTED);
      ...
      if (user_res_s == "y") {
        kafka_server_IP = utils.getJasmineGraphProperty(
        ...
        for (it = vec.begin(); it < vec.end(); it++) {
          ...
          if (item.length() > 0 && !(item.rfind("#", 0) == 0)) {
            ...
            if (vec2.at(0).compare("kafka.host") == 0) {
              if (item.substr(item.length() - 1, item.length()).compare("=") !=
                  0) {
      ...
      frontend_logger.log("Start serving `" + ADD_STREAM_KAFKA + "` command",
                          "info");
      ...
      int con_result_wr =
          write(connFd, con_message.c_str(), con_message.length());
      if (con_result_wr < 0) {
        frontend_logger.log("Error writing to socket", "error");
        loop = true;
        continue;
      ...
      Partitioner graphPartitioner(numberOfPartitions, 1,
                                   spt::Algorithms::HASH);
      ...
      input_stream_handler =
          thread(listen_to_kafka_topic, kstream, std::ref(graphPartitioner),
                 std::ref(workerClients));

    } else if (line.compare(STOP_STREAM_KAFKA) == 0) {
      frontend_logger.log("Start serving `" + STOP_STREAM_KAFKA + "` command",
                          "info");
      //          Unsubscribe the kafka consumer.
      kstream->Unsubscribe();
      string message =
          "Successfully stop `" + stream_topic_name + "` input kafka stream";
      int result_wr = write(connFd, message.c_str(), message.length());
      if (result_wr < 0) {
        frontend_logger.log("Error writing to socket", "error");
        loop = true;
      ...
      if (JasmineGraphFrontEnd::graphExistsByID(graphID, sqlite)) {
        frontend_logger.log(
            "Graph with ID " + graphID + " is being deleted now", "info");
        ...
        frontend_logger.log(
            "Graph does not exist or cannot be deleted with the current hosts "
            "setting",
            "error");
      ...
      if (utils.fileExists(path)) {
        ...
        frontend_logger.log(
            "Reformatted files created on "
            "/home/.jasminegraph/tmp/JSONParser/output",
            "info");
      ...
      if (!JasmineGraphFrontEnd::graphExistsByID(graph_id, sqlite)) {
        ...
        if (threadPriority > Conts::DEFAULT_THREAD_PRIORITY) {
          ...
          long graphSLA = JasmineGraphFrontEnd::getSLAForGraphId(
              sqlite, perfSqlite, graph_id, TRIANGLES,
              Conts::SLA_CATEGORY::LATENCY);
          jobDetails.addParameter(Conts::PARAM_KEYS::GRAPH_SLA,
                                  std::to_string(graphSLA));
        }
        ...
        jobDetails.addParameter(Conts::PARAM_KEYS::CATEGORY,
                                Conts::SLA_CATEGORY::LATENCY);
        if (canCalibrate) {
        ...
        std::string errorMessage =
            jobResponse.getParameter(Conts::PARAM_KEYS::ERROR_MESSAGE);
        ...
        std::string triangleCount =
            jobResponse.getParameter(Conts::PARAM_KEYS::TRIANGLE_COUNT);
        ...
        frontend_logger.log("Triangle Count: " + triangleCount +
                                " Time Taken: " + to_string(msDuration) +
                                " milliseconds",
                            "info");
        result_wr =
            write(connFd, triangleCount.c_str(), triangleCount.length());
        if (result_wr < 0) {
          frontend_logger.log("Error writing to socket", "error");
        }
        result_wr = write(connFd, "\r\n", 2);
      ...
      if (!JasmineGraphFrontEnd::graphExistsByID(graph_id, sqlite)) {
        ...
        string sqlStatement =
            "SELECT vertexcount from graph where idgraph=" + graph_id;
        ...
        result_wr = write(connFd, to_string(vertexCount).c_str(),
                          to_string(vertexCount).length());
        if (result_wr < 0) {
          frontend_logger.log("Error writing to socket", "error");
        }
        result_wr = write(connFd, "\r\n", 2);
      ...
      if (!JasmineGraphFrontEnd::graphExistsByID(graph_id, sqlite)) {
        ...
        string sqlStatement =
            "SELECT edgecount from graph where idgraph=" + graph_id;
        ...
        result_wr = write(connFd, to_string(edgeCount).c_str(),
                          to_string(edgeCount).length());
        if (result_wr < 0) {
          frontend_logger.log("Error writing to socket", "error");
        }
        result_wr = write(connFd, "\r\n", 2);
      ...
      Utils utils;
      std::string federatedEnabled =
          utils.getJasmineGraphProperty("org.jasminegraph.federated.enabled");
      ...
      std::vector<std::string>::iterator itr =
          std::find(trainargs.begin(), trainargs.end(), "--graph_id");
      std::string graphID;
      ...
      if (federatedEnabled == "true") {
        JasmineGraphServer *jasmineServer = new JasmineGraphServer();
        if (utils.getJasmineGraphProperty("org.jasminegraph.fl.org.training") ==
            "true") {
        ...
        JasminGraphTrainingInitiator *jasminGraphTrainingInitiator =
            new JasminGraphTrainingInitiator();
        jasminGraphTrainingInitiator->initiateTrainingLocally(graphID,
                                                              trainData);
      }
      ...
      if (JasmineGraphFrontEnd::isGraphActiveAndTrained(graphID, sqlite)) {
        if (utils.fileExists(path)) {
          ...
          JasminGraphLinkPredictor *jasminGraphLinkPredictor =
              new JasminGraphLinkPredictor();
          jasminGraphLinkPredictor->initiateLinkPrediction(graphID, path,
                                                           masterIP);
        } else {
          frontend_logger.log(
              "Graph edge file does not exist on the specified path", "error");
          continue;
        }
      } else {
        frontend_logger.log(
            "The graph is not fully accessible or not fully trained.", "error");
        continue;
      }
    } else if (line.compare(START_REMOTE_WORKER) == 0) {
      int result_wr =
          write(connFd, REMOTE_WORKER_ARGS.c_str(), REMOTE_WORKER_ARGS.size());
      if (result_wr < 0) {
        frontend_logger.log("Error writing to socket", "error");
      }
      result_wr = write(connFd, "\r\n", 2);
      ...
      JasmineGraphServer *jasmineServer = new JasmineGraphServer();
      bool isSpawned = jasmineServer->spawnNewWorker(
          host, port, dataPort, profile, masterHost, enableNmon);
      ...
      std::vector<vector<pair<string, string>>> categoryResults =
          perfSqlite.runSelect("SELECT id FROM sla_category where command='" +
                               command_info + "';");

      string slaCategoryIds;

      for (std::vector<vector<pair<string, string>>>::iterator i =
               categoryResults.begin();
      ...
      string adjustedIdList =
          slaCategoryIds.substr(0, slaCategoryIds.size() - 1);

      std::stringstream ss;
      std::vector<vector<pair<string, string>>> v = perfSqlite.runSelect(
          "SELECT graph_id, partition_count, sla_value FROM graph_sla where "
          "id_sla_category in (" +
          adjustedIdList + ");");
      for (std::vector<vector<pair<string, string>>>::iterator i = v.begin();
           i != v.end(); ++i) {
        ...
        for (std::vector<pair<string, string>>::iterator j = (i->begin());
          ...
          if (counter == 0) {
            ...
            std::string graphQuery =
                "SELECT name FROM graph where idgraph='" + graphId + "';";
            std::vector<vector<pair<string, string>>> graphData =
                sqlite.runSelect(graphQuery);
            if (graphData.size() == 0) {
  ...
  frontend_logger.log(
      "Closing thread " + to_string(pthread_self()) + " and connection",
      "info");
  close(connFd);
  return NULL;
}
...
int JasmineGraphFrontEnd::run() {
  ...
  while (true) {
    ...
    // TODO(miyurud):Temporarily commenting this line to enable building the
    // project. Asked tmkasun to provide a
    //  permanent fix later when he is available.
    threadVector.push_back(std::thread(frontendservicesesion, masterIP, connFd,
                                       this->sqlite, this->perfSqlite,
                                       this->jobScheduler));
...
bool JasmineGraphFrontEnd::graphExists(string path, SQLiteDBInterface sqlite) {
  ...
  string stmt = "SELECT COUNT( * ) FROM graph WHERE upload_path LIKE '" + path +
                "' AND graph_status_idgraph_status = '" +
...
/**
 * This method checks if an accessible graph exists in JasmineGraph with the
 * same unique ID.
 * @param id
 * @param dummyPt
 * @return
 */
bool JasmineGraphFrontEnd::graphExistsByID(string id,
                                           SQLiteDBInterface sqlite) {
  bool result = true;
 ...
 */
void JasmineGraphFrontEnd::removeGraph(std::string graphID,
                                       SQLiteDBInterface sqlite,
                                       std::string masterIP) {
  vector<pair<string, string>> hostHasPartition;
  vector<vector<pair<string, string>>> hostPartitionResults = sqlite.runSelect(
      "SELECT name, partition_idpartition FROM worker_has_partition INNER JOIN "
      "worker ON "
      "worker_has_partition.worker_idworker = worker.idworker WHERE "
      "partition_graph_idgraph = " +
      graphID + ";");
  ...
  sqlite.runUpdate("UPDATE graph SET graph_status_idgraph_status = " +
                   to_string(Conts::GRAPH_STATUS::DELETING) +
  ...
  sqlite.runUpdate(
      "DELETE FROM worker_has_partition WHERE partition_graph_idgraph = " +
      graphID);
 ...
 * @return
 */
bool JasmineGraphFrontEnd::isGraphActiveAndTrained(std::string graphID,
                                                   SQLiteDBInterface sqlite) {
  bool result = true;
 ...
 * @return
 */
bool JasmineGraphFrontEnd::isGraphActive(std::string graphID,
                                         SQLiteDBInterface sqlite) {
  bool result = false;
  string stmt = "SELECT COUNT( * ) FROM graph WHERE idgraph LIKE '" + graphID +
                "' AND graph_status_idgraph_status = '" +
...
void JasmineGraphFrontEnd::getAndUpdateUploadTime(std::string graphID,
                                                  SQLiteDBInterface sqlite) {
  struct tm tm;
  vector<vector<pair<string, string>>> uploadStartFinishTimes =
      sqlite.runSelect(
          "SELECT upload_start_time,upload_end_time FROM graph WHERE idgraph = "
          "'" +
          graphID + "'");
  ...
  sqlite.runUpdate("UPDATE graph SET upload_time = " + to_string(difTime) +
                   " WHERE idgraph = " + graphID);
  frontend_logger.log("Upload time updated in the database", "info");
}

JasmineGraphHashMapCentralStore JasmineGraphFrontEnd::loadCentralStore(
    std::string centralStoreFileName) {
  frontend_logger.log(
      "Loading Central Store File : Started " + centralStoreFileName, "info");
...
map<long, long> JasmineGraphFrontEnd::getOutDegreeDistributionHashMap(
...
long JasmineGraphFrontEnd::getSLAForGraphId(
  ...
  string graphSlaQuery =
      "select graph_sla.sla_value from graph_sla,sla_category where "
      "graph_sla.id_sla_category=sla_category.id "
  ...
  std::vector<vector<pair<string, string>>> slaResults =
      perfSqlite.runSelect(graphSlaQuery);
...
void JasmineGraphServer::pageRank(std::string graphID, double alpha,
  ...
  std::map<std::string, JasmineGraphServer::workerPartition>::iterator
      workerIter;
  for (workerIter = graphPartitionedHosts.begin();
       workerIter != graphPartitionedHosts.end(); workerIter++) {
  ...
  for (workerIter = graphPartitionedHosts.begin();
    ...
    bzero(data, 301);
    int result_wr =
        write(sockfd, JasmineGraphInstanceProtocol::PAGE_RANK.c_str(),
              JasmineGraphInstanceProtocol::PAGE_RANK.size());
    if (result_wr < 0) {
      frontend_logger.log("Error writing to socket", "error");
    }

    frontend_logger.log("Sent : " + JasmineGraphInstanceProtocol::PAGE_RANK,
                        "info");
    bzero(data, 301);
    ...
    if (response.compare(JasmineGraphInstanceProtocol::OK) == 0) {
      frontend_logger.log("Received : " + JasmineGraphInstanceProtocol::OK,
                          "info");
    } else {
    ...
    frontend_logger.log("Sent : Partition ID " + std::to_string(partitionID),
                        "info");
    ...
    result_wr = write(sockfd, std::to_string(graphVertexCount).c_str(),
                      std::to_string(graphVertexCount).size());
    ...
    frontend_logger.log(
        "graph vertex count: " + std::to_string(graphVertexCount), "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    response = utils.trim_copy(response, " \f\n\r\t\v");
    ...
    result_wr = write(sockfd, std::to_string(alpha).c_str(),
    ...
    frontend_logger.log("page rank alpha value sent : " + std::to_string(alpha),
                        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
    ...
    result_wr = write(sockfd, std::to_string(iterations).c_str(),
    ...
    frontend_logger.log(
        "page rank iterations value sent : " + std::to_string(iterations),
        "info");
    bzero(data, 301);
    read(sockfd, data, 300);
    response = (data);
...
bool JasmineGraphFrontEnd::modelExists(string path, SQLiteDBInterface sqlite) {
  ...
  string stmt = "SELECT COUNT( * ) FROM model WHERE upload_path LIKE '" + path +
                "' AND model_status_idmodel_status = '" +
                to_string(Conts::GRAPH_STATUS::OPERATIONAL) + "';";
  std::vector<vector<pair<string, string>>> v = sqlite.runSelect(stmt);
  int count = std::stoi(v[0][0].second);
  if (count == 0) {
...
bool JasmineGraphFrontEnd::modelExistsByID(string id,
                                           SQLiteDBInterface sqlite) {
  bool result = true;
